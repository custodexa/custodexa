// 政策鍵域歸屬單一事實源（settings-domain-restructure D5/D7）：
// 四個設定頁各吃自己的 sections；一致性單測斷言四域鍵集互斥且聯集＝後端全鍵集。
// 後端新增政策鍵時必須在此歸域，未歸域的鍵只會落到安全政策母頁「其他」區塊。
// title/hint 譯文住 locale 檔 policyDomain.section.<id>.*（i18n-foundation D6），
...
export const SECURITY_SECTIONS = [
  ...
  section('log_retention', [
    ...
    // 檢查點鏈保留（audit-checkpoint-chain D9）：受跨鍵約束——不得短於上列
    ...
    // 鏈自動驗證三鍵（audit-chain-scheduled-verification）：緊接封存門檻之後，
    ...
    // 單輪清理刪除上限（policy-numeric-lower-bounds）：接在上述封存與驗證之後，
  ...
  // 叢集存取（policy-numeric-lower-bounds）：K8s 資產列表的逾時預算。
...
export const ACCESS_SECTIONS = [
  ...
  // 資料傳輸管控（data-transfer-control D1）：剪貼簿雙向需重連才生效，
...
export const KEY_SECTIONS = [
  section('key', [
    ...
    // 單輪換鑰重加密上限（policy-numeric-lower-bounds）：調小才危險
    //（換鑰永遠跑不完＝輪替實質失效），下界由後端 Min 承擔
    'key_rotation_max_per_run',
  ]),
]

// 母頁分域偏離列表用（settings-domain-restructure D5）：label 顯示名、route 跳轉
