...
describe('ToolCallLedger', () => {
  it('被拒列呈現原因碼與說明文字', async () => { const w = await open(); expect(w.get('[data-test="denial-code"]').text()).toBe(row.denial_code); expect(w.get('[data-test="denial-explanation"]').text()).toBe(t('apiError.AUTH_REQUEST_ITEM_MISMATCH')); expect(w.text()).toContain('sha256-fixture'); expect(w.text()).toContain('[REDACTED]'); expect(w.find('script').exists()).toBe(false) })
  ...
  it('遮罩筆數非零列免展開可辨', async () => { const w = await open(); expect(w.get('[data-test="masked-count"]').text()).toBe('2'); expect(w.get('[data-test="masked-count"]').element.closest('details')).toBeNull(); expect(w.get('details').attributes('open')).toBeUndefined() })
  it('對象欄由會話帶出資產與帳號，無會話與查無會話各自可辨', async () => {
    api.list.mockResolvedValue({ data: [{ ...row, id: 1, session_id: 5 }, { ...row, id: 2, session_id: 6 }, { ...row, id: 3, session_id: null }], total: 3 })
    const w = await open({ targets: { 5: { asset: 'app-web-01', account: 'appuser' } } })
    const targets = w.findAll('[data-test="ledger-target"]')
    expect(targets[0].text()).toContain('app-web-01'); expect(targets[0].text()).toContain('appuser')
    expect(targets[1].text()).toBe('—')
    expect(targets[2].text()).toBe(t('agentLedger.noSession'))
  })
  it('會話頁的連結欄指回任務', async () => {
    api.list.mockResolvedValue({ data: [{ ...row, session_id: 5, access_request_id: 8 }], total: 1 })
    const w = await open({ query: { session_id: 5 }, showSession: false })
    expect(w.get('[data-test="ledger-row"] a').attributes('href')).toBe('/audit/agent-tasks/8')
    expect(w.text()).toContain(t('agentLedger.task', { id: 8 }))
  })
  it('首層不露紀錄序號，序號只在展開層', async () => {
    api.list.mockResolvedValue({ data: [{ ...row, seq: 987654 }], total: 1 })
    const w = await open()
    expect(w.get('.tool-ledger__summary').text()).not.toContain('987654')
    expect(w.get('details').text()).toContain('987654')
  })
