<template>
  <span class="principal-badge">
    ...
    >{{ t('agentPrincipals.ownerValue', { owner: ownerName || (ownerId ? `#${ownerId}` : t('agentPrincipals.unavailable')) }) }}</span>
  </span>
</template>
<script setup>
import { t } from '@/i18n'
defineProps({ kind: { type: String, default: '' }, ownerId: { type: Number, default: null }, ownerName: { type: String, default: '' } })
</script>
<style scoped>
