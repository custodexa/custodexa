...
describe('AgentBreakers 事件序列', () => {
  it('事件序列逐筆呈現', async () => {
    ...
    const w = await open(); expect(w.findAll('[data-test="probe-event"]')).toHaveLength(4); expect(w.text()).toContain('不同目標：1'); expect(w.text()).toContain('因熔斷停用於')
  ...
  it('文案不斷言成因', async () => { const w = await open(); expect(w.text()).not.toMatch(/惡意|攻擊|逃避/); expect(w.text()).toContain('不是目前時間窗的熔斷計數') })
