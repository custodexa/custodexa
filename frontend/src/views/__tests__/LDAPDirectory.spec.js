// LDAP 目錄設定頁（ldap-settings-migration 4.1／4.2）。
//
...
describe('LDAPDirectory 設定頁', () => {
  ...
  // UI 審查 H3：原本只放一則 alert 就宣稱「擋在表單之前」，但儲存鈕照樣可按。
  ...
  // UI 審查 H4：警示與狀態帶原本逐字相同（標題與內文都是），上下相鄰。
  ...
  // UI 審查 M7：warn 閘只在存檔那一刻確認一次，之後回到本頁就再也看不到風險，
  ...
  // UI 審查 M1：勾選後欄位雖 disabled，placeholder 原本仍寫「留空則不修改」，
  ...
  it('測試成功：逐階段列出通過，並顯示命中筆數與屬性抽樣', async () => {
    ...
    // UI 審查 M5：缺值屬性原本與「測試目標」同色同重，在一片綠色的「通過」
  ...
  // UI 審查 M2：原本只 v-for 回應帶回來的階段，撥號失敗時整份清單只剩一行。
  ...
  // UI 審查 H6：儲存才是主要動作，但它原本拿到共用碼那句「設定含不安全傳輸」
  ...
  // NEW-H1：H3 只擋了「讀取失敗」，沒擋「讀取還沒回來」。action-bar 在 el-form
  ...
  // NEW-H2：三階全過但命中 0 筆＝這份設定一個使用者都登不進來。原本頭條是
  // 綠色「通過」，唯一的線索是次要灰字的「搜尋命中 0 筆」——與 M5 同一條規則
  ...
  // NEW-H3：動作列在三張分區卡之後，從頁尾按儲存被驗證擋下時，視窗內毫無變化
  ...
  // NEW-M2：欄位只是 disabled，先打字再勾選會讓 model 仍留著那串字，於是
  ...
  // NEW-M3：工作流是「先測後存」，結果卡因此會與後續編輯並存；改完 URL 後
  ...
  // NEW-M4：整頁表單，切選單或按「重新整理」都會靜默丟掉草稿；重新整理這個
  ...
  // NEW-M1：原文案只說「調整傳輸政策等級」，而選單裡沒有任何叫「傳輸政策」
  ...
  // R3-H1：H5 立的規則是「不知道就不能說『尚未設定』」，輪 1 只把它套到 OIDC
  ...
  // R3-M1：NEW-M3 的過期聲明拿整份 payload 比較，於是切一下「啟用目錄登入」
