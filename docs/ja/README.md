...
すべての接続はポリシーを通ってから開きます。残るのは録画とコマンドの記録で、監査人がオフラインで検証できる
署名つきの証拠パッケージにまとまります。
...
実行してください。Linux ホストでは、最初の `up` の前に録画ディレクトリを準備します（[サービスの起動](QUICKSTART.md#3-サービスの起動)を参照）。
Windows では WSL の中でスクリプトを実行してください。
...
MCP サーバーは Custodexa のバックエンド自身が `POST /api/v1/mcp` で streamable HTTP として
提供するため、別途デプロイするものはありません。

streamable HTTP に対応したホストは `https://<お使いの Custodexa>/api/v1/mcp` へ直接接続し、
agent token を `Authorization: Bearer` で送ります。stdio でローカルのサーバーを起動することしか
できないホストは、代わりに [`custodexa-mcp`](https://github.com/custodexa/custodexa-mcp) アダプターを
使います。このアダプターは、すべての呼び出しを同じエンドポイントへ転送するだけで、自身では何も判定しません。
インストールは `go install github.com/custodexa/custodexa-mcp@latest` で行うか、
[Releases](https://github.com/custodexa/custodexa-mcp/releases) からバイナリを入手してください。

agent チャネルで許される範囲は次のとおりです。

- **agent のセルフサービス作成は出荷時に無効です。** agent は管理者が作成し、それぞれに人間の
  責任者を指定します。セキュリティポリシーでセルフサービス作成を許可すると、有効な人間のユーザーは
  だれでも自分が責任者となる agent を作成できます。
- **agent token には必ず有効期限があります。** 平文が表示されるのは発行時の一度だけです。
- **agent が接続できるのは、`request_access` で申請して承認された資産だけです。** 「承認者の承認が必要」の資産では、
  申請は人間の承認者の判断を待ちます。「申請不要」と「理由の入力で接続可」の資産は申請の時点で承認され、
  出荷時の既定は「申請不要」です。
- **ツール呼び出しはすべて記録されます。** ツール呼び出し台帳は引数と判定を保存し、拒否された呼び出しも
  含みます。agent 自身のタスクやセッションに結びつけられない呼び出しは拒否され、HTTP リクエストの監査に
  記録されます。
- **agent へ返す内容はマスクされ、録画には元の画面が残ります。** マスクされるのは有効な出力ルールに
  一致した部分だけで、出荷時のルールはカード番号と秘密鍵のヘッダーを対象とします。

10 種類のツールとその挙動は [docs/API_SPEC.md](../API_SPEC.md) の MCP の章にあります。
