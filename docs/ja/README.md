...
<p align="center"><a href="https://custodexa.org/en/">公式サイト</a> · <a href="https://custodexa.org/en/docs/quickstart/">オンラインドキュメント</a></p>
<p align="center">
...
オープンソースの踏み台ホスト（Bastion Host）です。サーバーやデータベースへの特権的な接続を
すべて単一のゲートウェイへ集約し、セッションは録画され、コマンドは記録されます。
...
- **完全なオープンソース、単一エディション。** エンタープライズ版も、有料機能もありません。
  見えているものがすべてです（AGPL-3.0）。
- 8 種類のプロトコルをひとつの操作体験で。SSH、RDP、VNC、MySQL、PostgreSQL、SQL Server、
  Redis、Kubernetes exec が、いずれもブラウザのタブひとつで開きます。
- **監査を第一に。** すべてのプロトコルで全セッションを録画し、再生（シーク、速度調整）
  できます。コマンド単位の監査は vim のような全画面プログラムも正しく扱い、クリップボードと
  ファイル転送の内容も記録されます。危険なコマンドはリアルタイムで警告または遮断でき、
  Webhook で通知できます。
- 資格情報はバックエンドの外へ出ません。接続はバックエンドのプロキシが開始し、ワンタイムの
  接続トークン、ホストキー検証、資格情報のローテーション計画を備えます。既定では、
  プラットフォーム自身のマスターキーもメモリ上にのみ存在し、ブラウザから解封（unseal）します
  （無人運用向けに env／KMS モードへ切り替えられます）。
- 既存の環境になじみます。LDAP ログイン、多要素認証（TOTP）、そして「どのマシンのどの
  アカウントを誰が使ってよいか」まで踏み込んだロールベースのアクセス制御。
- **導入は単純です。** docker compose のコマンドひとつ、本番構成はコンテナ 4 つ、稼働後は
  外部への通信を必要としません。
