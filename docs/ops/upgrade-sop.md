...
### 2.0 先確定回退方案，並確認本節的前提成立

**在動手之前先讀第 4 節。本版不提供 migration 回滾。**
升級失敗時唯一的回退手段是還原備份，其代價是遺失「最近一次備份之後產生的全部資料」。
請據此決定備份時點與停機視窗，而不是升級到一半才發現沒有退路。

> **本節程序的適用前提（1.0 的硬邊界）**：`Custodexa 1.0` 的資料庫 schema 由**單一 baseline**
> （`20260816_schema_baseline`）建立，逐條增量 migration 已不存在，因此
> **本版不提供「既有資料庫直接接上新二進位」的就地升級路徑**。
>
> 具體地說：若資料庫的 `schema_migrations` 表內含有本版程式碼不認識的版本值，
> 而 baseline 尚未套用，後端會**拒絕啟動**（見 §2.6）。這使得本節的「部署新版本」
> 只適用於**同屬 1.0 baseline 世代**的版本更替。跨越 baseline 的升級請視為新裝
> ＋資料移轉專案，不在本 SOP 範圍內，且**產品未提供任何移轉工具**。
>
> 這不是待補的功能缺口，是 1.0 的交付邊界。請在規劃升級前確認貴方的來源版本。
...
`guacamole/guacd:1.6.0`、`postgres:16.15-alpine3.24`），**這不是潔癖**：
...
> **這個指標的誠實邊界**：它讀的是佇列本身的長度，代表「還有多少列尚未被 worker 取走」。
...
這是 1.0 刻意的 fail-close，**不是故障，也不是資料庫損毀**。
...
1. **開發環境**：重建一個空的資料庫即可。
2. **正式環境**：**本版不提供就地升級路徑**。可行的只有「以新資料庫做新裝」，
   而既有資料的移轉**沒有產品工具支援**，屬專案性工作。請在此停下並與交付方確認，
   不要自行嘗試繞過。
...
- 上述都做了仍可能有極少量遺失——這是本版的已知邊界，不是操作失誤。

---

## 4. 回退路徑（誠實界定）

### 4.1 本版不提供 migration 回滾

**升級後若要退回舊版本，唯一的手段是還原備份。**

**本版的 migration 只有一條，但這不代表「只有一條所以回滾很簡單」——正好相反：**

- **本版唯一的 migration 是整個 schema baseline，而它的 `Down` 一律回拒絕錯誤。**
  這是刻意的。baseline 建的是**整個資料庫 schema**，「回滾」它等於刪掉全部資料表
  ——使用者、資產、授權與審計證據一併消失，而不是還原到某個較舊的 schema 形狀。
  **一個看起來像回滾入口、實際上是資料庫毀滅按鈕的東西，比沒有入口更危險**，
  故該路徑一律回錯，把退路指回還原備份。
- 程式碼中仍存在一個內部的 `RollbackMigration` 函式，但**沒有任何生產可用的入口**：
  沒有 CLI 旗標，沒有 API 端點，沒有管理介面按鈕，產品程式碼中也沒有任何呼叫者
  （現存的唯一呼叫者是一支**斷言「回滾必被拒絕」的測試**）。
  它保留的用途是「baseline 之後若新增增量 migration，
  屆時走同一條路徑」，對本版而言它是一個**產品內跑不到、且就算跑到也只會回錯**的函式。

**請不要把這個函式的存在理解成「本版可以回滾 migration」。它不能。**
規劃升級的回退方案時，唯一可以列入選項的是「還原備份」。

**另請注意 §2.0／§2.6 的那條硬邊界**：跨越 baseline 世代的資料庫連新版都啟動不了，
故那個情境下連「升級失敗要不要回退」都不會發生——它在動到資料之前就被擋住了。
