...
## Agent disclosure records after an upgrade

The `agent_visibility_exposures` migration starts recording asset identifiers disclosed to agent principals: only the asset IDs actually returned on a filtered list page, and IDs approved for the task executor. Human requests do not populate this table. Records are retained when access is removed; they are disclosure evidence, not a reconstruction of group membership or historical authorization.

For denied references, a missing or soft-deleted asset is `retired`; an existing asset with an exposure record is `revoked`; an existing asset without one is `never_visible`. Only `never_visible` counts toward the breaker, with each ID counted once per window. All three return the same 404.

Pre-upgrade sessions and group history are not backfilled. An ID learned before the upgrade and not subsequently listed again or approved is classified as `never_visible` whenever a reference is denied until an exposure record is created. **This is not limited to the first time window; an administrator can release the incident with a reason.** Release clears the pending marker but does not revive suspended tokens; issue a new token afterward. Keep the exposure table when rolling back by disabling its readers and writers. Notifications use existing webhook/Slack channels with `owner_id`; this does not guarantee personal delivery.
...
### Agent audit chain: verification and rollback boundary

New checkpoints use payload v3. Verify old v1/v2 checkpoints with their original encoding. The tool-call chain commits to row count, ID range and immutable first-stage fields, including intended content and recording time; results are protected by each row's HMAC, not by the checkpoint aggregate. Pending rows mean "recorded; whether sent and the result are both unknown" and may complete after sealing. A holder of the HMAC key can forge result fields; a successful chain check alone does not prove those results. A row-level HMAC proves that a key holder authenticated that version, not that it is the latest version or reflects what actually executed; anyone with direct database write access can replay an old row.

The terminal seed set contains 16 rules: the original 12 input rules, two output rules and two agent/input/block rules. Existing rows default to subject `all`; authenticated principal kind selects human/agent rules. Unknown kind still matches `all`. Check both agent seeds after migration; their inserts use `ON CONFLICT (name) DO NOTHING` and preserve administrator edits. These text rules cover enumerated forms only; file writes and equivalent operations through other tools still require task-scope and report review. They do not establish complete control of agent operations. An old matcher ignores subject and could apply agent rules to humans. Do not run an old binary against this database.

Before rollback, stop new work and retain a separate, consistent post-upgrade evidence backup: database, recordings, checkpoint signatures and the corresponding versioned HMAC keys. Keep tool-call rows (including pending), report versions, probe events and visibility exposures. Reports sent after task closure do not erase the original missing-report fact. The new breaker alert can have a NULL session_id; old readers cannot safely consume it. Down migrations refuse destructive removal of these records; disabling reads/writes is not a schema downgrade. Restore the pre-upgrade backup with the old images using §4.2. Keep the post-upgrade evidence for verification with a compatible v3 reader; never remove fields, rewrite snapshots, re-sign the chain or manufacture pending results to make old readers pass.

Visibility exposures begin at this upgrade; they do not reconstruct older visibility. An identifier learned before the upgrade and not subsequently listed again or approved is classified as never_visible whenever a reference is denied until an exposure record is created, and can trip the breaker; this is not limited to the first time window. An administrator or owner can release the incident with a reason, but release does not reactivate tokens. Notifications use the existing webhook/Slack channels with owner_id, not a private inbox. Command coverage now reports incomplete and degraded_count independently of retention; zero degraded rows omits both fields and cannot prove that no original commands were lost.
