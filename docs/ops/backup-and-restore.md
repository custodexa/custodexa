...
**不屬備份對象的暫存目錄**：證據包非同步匯出的產物落於 `/var/lib/custodexa/exports`
（`EXPORT_ARTIFACT_PATH`，容器內路徑）。**它是暫存、非備份對象**：產物於保留期（24h）後
由系統自動清除，過期即下載失效——備份它沒有意義，其內容也隨時可由申請者重新發起匯出取回。
本目錄**預設不掛 bind mount**（不在上表三落點內），隨容器生命週期存放；不要為它另設定備份，
也不要期待它跨容器重建後仍在。**注意其敏感度**：證據包產物含**解密後的剪貼簿明文**與錄影本體
...
系統以信封加密保護資產憑證、LDAP bind 密碼、通知通道 URL 與密鑰、簽章私鑰、
**剪貼簿審計內容**等欄位。**資料庫備份還原後能不能解開，完全取決於 KEK（金鑰加密金鑰）能不能取回。**
