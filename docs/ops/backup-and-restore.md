...
本版**未隨產品交付任何備份或還原腳本**。備份由部署方以標準工具執行，本文提供程序。
這是刻意的界定：備份保存位置、保留週期、加密與異地複製屬部署方的資料治理範圍，
產品不代管，也就不應假裝代管。
...
**全部為 bind mount，無 named volume**。兩個直接後果：
...
**未設定時退回相對路徑 `logs/audit_fallback`**——相對於行程的工作目錄，不在
`DATA_PATH` 之下。
...
系統以信封加密保護資產憑證、LDAP bind 密碼、通知通道 URL 與密鑰、簽章私鑰等欄位。
**資料庫備份還原後能不能解開，完全取決於 KEK（金鑰加密金鑰）能不能取回。**
...
- **多區金鑰（MRK）的已知限制**：落庫識別含 region。**切換到 replica 等同換鑰，必須重包**；
  本版不利用 MRK 的原生跨區解密能力。災難復原計畫若假設「切到 replica 區就能起來」，
  該假設在本版不成立。
...
| 6 | **金鑰清冊指紋比對** | 管理端「金鑰管理」頁 | `JWT_SECRET`、KEK、匯出簽章鑰三者的指紋**與備份前記錄的值相同** |
| 7 | 加密欄位可解 | 開啟任一具憑證的資產、或觸發一次 LDAP 登入 | 不出現解密失敗 |
| 8 | 錄影可播 | 開啟一筆備份前既有的會話錄影 | 能播放（驗證錄影目錄與資料庫的對應仍成立） |
| 9 | 審計鏈驗證 | 稽核端的完整性驗證頁 | 驗證結果與備份前一致 |

> 第 6 項是最有價值的一項：**指紋是單向摘要，比對相同即可確認還原後用的是同一把金鑰**，
> 而不必接觸任何金鑰材料。請在每次備份時一併記錄這三個指紋，隨備份保存。
