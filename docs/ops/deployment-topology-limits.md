...
「不支援多實例」在此是字面意思——不是「支援但尚未最佳化」，也不是「可以跑但效能較差」。
...
- 以滾動更新（rolling update）方式升級——那在切換過程中必然出現新舊兩個實例並存。
...
- **漏洞掃描對該映像的結果不可作為安全依據。** 掃描它會得到「零發現」，
  但那是**沒有資料**而非沒有問題：已過支援期的系統不再有漏洞資料庫更新，
  且 guacd 本體是編譯後的二進位、不在套件清單內，掃描器看不到它。
  **把這個零當成安全證明，是本文要明確阻止的判讀。**
...
該路徑仍然存在。**若你的漏洞管理政策要求所有元件的基礎系統都在支援期內，
本版無法滿足該條**——這是本文要讓你在部署前就知道的事，不是要你接受的事。
...
本版不提供應用層的高可用機制。可用性目標若要求自動故障接管，本版無法滿足；
可行的規劃方向是縮短還原時間——確保備份與 KEK 材料就緒、且回存程序已演練過。
見[備份與還原](./backup-and-restore.md)。
