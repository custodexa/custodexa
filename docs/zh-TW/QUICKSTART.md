...
[English](../QUICKSTART.md) | **繁體中文** | [日本語](../ja/QUICKSTART.md) | [其他語言 →](../README.md)

> 相關文件：API 參考見 [API_SPEC.md](../API_SPEC.md)、資料庫參考見 [DB_SCHEMA.md](../DB_SCHEMA.md)、專案總覽見 [README.md](README.md)（英文版在[repo 根目錄](../../README.md)）、貢獻與開發流程見 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
```bash
docker compose ps
# 正式版（預設）：backend, frontend, postgres, guacd, tls-proxy
#   （tls-init 準備憑證後即結束，狀態顯示為 exited(0) 是正常的）
...
| | 正式版（預設） | 開發版 |
|---|---|---|
| 前端 | https://localhost （TLS 代理；http://localhost 會 301 導過去） | http://localhost:3000 （Vite dev server） |
| 後端 | 不對外開埠（經代理與 nginx 轉發 `/api`） | http://localhost:8080 |

正式版對外只有 TLS 代理的兩個埠，`443`（https）與 `80`（http，只做導向），
瀏覽器預設就是這兩個埠，網址不必帶埠號。主機上這兩個埠已經被別的服務占用時，
在 `.env` 改設另一組——`TLS_HTTPS_PORT=8443` 與 `TLS_HTTP_PORT=8088` 是常見的選擇——
網址就要跟著帶埠，像 `https://localhost:8443`。以下各段一律以預設埠為準。
憑證來源預設是產品自己產生的本地 CA（`TLS_MODE=selfsigned`），故瀏覽器在你把 CA
派發到使用者端之前會顯示警告；自備憑證與派發 CA 的做法見「對外傳輸加密（TLS）」。
...
開 `https://localhost/`（開發版為 `http://localhost:3000`）。
...
`GIN_MODE=release`），對外只有 TLS 代理的 `443`（https）與 `80`（http 導向）。
...
| `TLS_DOMAIN` | 對外主機名，會寫進憑證；留空等同 `localhost` |
| `PUBLIC_BASE_URL` | 對外 https 位址（OIDC 的 `redirect_uri` 吃這個值），與 `TLS_DOMAIN` 同網域 |
...
# (3) 前端可達，且 http 會導向 https（自簽憑證時 curl 加 -k）
...
> **對外連線預設就是 TLS**：正式版內建反向代理，`443` 走 https、`80` 一律導向它。
> 憑證預設由產品自己簽（本地 CA），把 CA 派發到使用者端後瀏覽器即顯示為受信任；
> 有機構憑證或自家 ingress 的部署走另外兩條路，三者都在下方「對外傳輸加密（TLS）」。
...
正式版的對外連線由內建的反向代理終結 TLS：`443` 提供 https，`80` 一律 301 導向它，
frontend 與 backend 都不發布 host 埠。代理提供 TLS 1.2+、HSTS 與 **WebSocket upgrade 轉發（wss）**，
設定檔不必動手改——網域取自 `.env` 的 `TLS_DOMAIN`，啟動時展開進設定。
`bash scripts/quickstart.sh` 另會把 `TRUSTED_PROXIES` 填成這套堆疊所在的 Docker 子網，
後端因此讀得到代理轉送過來的使用者端位址，而不是把每一筆請求都算到代理頭上。

憑證來源由 `.env` 的 `TLS_MODE` 決定，兩個值對應下面前兩個小節：`selfsigned`（預設，
產品自簽的本地 CA）與 `provided`（你自己的憑證）。已經有自家 ingress 的部署走第三條路，
用 overlay 關掉內建代理。

不論走哪一條，這兩件事不變：對外的 edge 須提供 TLS 1.2+、HTTP→HTTPS 導向、HSTS 與
WebSocket upgrade 轉發；edge 到本主機的 hop 若跨越不可信網段，該 hop 亦須加密（re-encrypt）。
應用本身已 TLS-ready：前端依頁面協定自動 `ws`↔`wss`、access token 走 Authorization header，
唯一要對齊的是 `PUBLIC_BASE_URL`（見「會話刷新 cookie 的 Secure 旗標」）。
...
#### 預設：產品自簽的本地 CA（`TLS_MODE=selfsigned`）

沒有機構 PKI 也能一條指令拿到 https。首次啟動時，系統在 `tls/` 產生一組本地 CA 與
伺服器憑證（憑證 825 天、CA 10 年），之後每次啟動沿用；連線全程加密，信任鏈由你把 CA
派發到使用者端建立。

1. `.env` 設對外主機名與基準網址（`bash scripts/quickstart.sh` 會依本機主機名與 IP 自動填好）：
...
2. `docker compose up -d`。憑證在第一次啟動時產生。

3. 下載 CA 憑證並派發到會連進來的機器：

   ```bash
   curl -sk https://jumper.example.internal/custodexa-ca.crt -o custodexa-ca.crt
   ```

   Windows 網域環境用群組原則（電腦設定 → Windows 設定 → 安全性設定 → 公開金鑰原則 →
   受信任的根憑證授權單位）派發，macOS／行動裝置用 MDM 設定描述檔，單機則手動匯入
...
`TLS_DOMAIN` 只是這套系統對外的主機名，內網 DNS 或使用者端 hosts 解析得到就夠，
不必是公開註冊的網域（例如 `jumper.bnc.prod`）；`PUBLIC_BASE_URL` 設成同一個主機名的
https 位址即可。

機構有自己的 CA 時，在本機產私鑰與憑證簽發請求（SAN 要含該主機名），送交簽發：
...
簽回來之後，把伺服器憑證與中繼憑證依序串成 `tls/fullchain.pem`（葉憑證在最前面），
私鑰留在 `tls/privkey.pem`，`TLS_MODE` 設 `provided` 後重啟。使用者端要信任該 CA，
通常由 AD 群組原則或 MDM 派發。SSO 用的識別提供者若也在內網、走 https 且用同一把 CA，
把 CA 憑證加進後端容器的信任存放區（見「OIDC／SSO 部署注意」的
`OIDC_ALLOWED_INTERNAL_HOSTS` 段）。

#### 自備憑證（`TLS_MODE=provided`）

已經有公開 CA 或機構 CA 簽發的憑證時：

1. 把憑證鏈與私鑰放到 `tls/fullchain.pem` 與 `tls/privkey.pem`。
2. `.env` 設 `TLS_MODE=provided`、`TLS_DOMAIN`（與憑證上的主機名一致）與
   `PUBLIC_BASE_URL=https://<主機名>`。
3. `docker compose up -d`。

缺任何一個檔案時啟動會停在憑證準備步驟並指名缺哪一個，代理不會帶著半套設定起來：
...
前面已經有雲端 LB、既有 nginx／Traefik 之類的 ingress 時，用 overlay 關掉內建代理，
由前端自己發布 HTTP 埠給你的 ingress 連：

```bash
docker compose -f docker-compose.yml -f docker-compose.external-ingress.yml up -d
```

在 `.env` 取消 `COMPOSE_FILE=docker-compose.yml:docker-compose.external-ingress.yml`
的註解之後，日常的 `docker compose up -d`／`ps`／`down` 就不必每次帶 `-f`。
此形態下前端發布 `80`（`.env` 的 `HTTP_PORT` 可改），對外 TLS 由你的 ingress 承載，
契約如本節開頭所列；`PUBLIC_BASE_URL` 請填使用者實際看到的 https 位址。

有兩件事屬於你的 ingress 而不是這套堆疊。**轉發的 Host 標頭要帶上使用者實際連的那個埠**
——不是 `443` 就要帶：後端以 `Origin` 對 `Host` 判定同源請求，`Host` 少了埠，已認證的請求
就會被當成跨來源，回應是 403 且 body 為空（登入與解封最先撞到）。nginx 在 HTTP/1.1 下寫
`proxy_set_header Host $http_host;`；監聽走 HTTP/2 時協定本身沒有 Host 標頭，改以 `$host`
與對外埠組出該值。另外**在 `.env` 把 `TRUSTED_PROXIES` 設成你的 ingress 位址或其網段**：
這個值決定每筆請求算在哪個位址上，也就決定了稽核記到什麼、登入限流又算在誰頭上。

#### 自訂反向代理設定

內建範本涵蓋一般需求。要改動時複製一份，指過去，原檔不動：
...
該檔可改的位置：`server_name`（預設由 `TLS_DOMAIN` 帶入）、憑證路徑、額外標頭
（HSTS 的 `max-age`、`includeSubDomains`／`preload` 依你的網域策略），以及 upstream
（同一 docker 網路用服務名 `frontend:80`）。改完 `docker compose up -d tls-proxy` 套用。

#### 主機安裝 nginx 的變體

不想跑代理容器、要用主機上既有 nginx 時：用 external ingress overlay 讓前端發布 HTTP 埠
（建議綁 loopback，把 overlay 內的映射改成 `127.0.0.1:8088:80`），設定沿用同一份範本，
upstream 改指 `127.0.0.1:8088`，`server_name` 與憑證路徑換成你的。
nginx 低於 1.25.1 不支援 `http2 on;` 指令，改寫成 `listen 443 ssl http2;` 即可。
其他反向代理（Caddy、Traefik、雲端 LB）滿足本節開頭那組契約即可，
`docker/reverse-proxy/Caddyfile.example` 是等效的 Caddy 版本。

#### 驗證 TLS 生效

```bash
docker compose ps                                  # tls-proxy 為 Up（tls-init 為 exited(0)）
...
再登入開一條 SSH 連線，瀏覽器 DevTools 的 Network 應看到 `wss://` 串流——
終端開不起來而頁面正常時，多半是 WebSocket upgrade 轉發沒生效。
...
### 埠號被佔用
```bash
lsof -i :443   # 正式版 https（TLS 代理）
lsof -i :80    # 正式版 http 導向
lsof -i :3000  # 開發版前端
lsof -i :8080  # 開發版後端
# 停止佔用的程序，或在 .env 把 TLS_HTTPS_PORT 與 TLS_HTTP_PORT 改成沒被占用的一組
# （8443 與 8088 是常見的選擇）；`bash scripts/quickstart.sh --up` 會在啟動任何東西之前
# 指名擋路的程式
```
...
- 查看 [README.md](README.md)（繁中）或 [README.md](../../README.md)（英文）了解專案架構與文檔地圖
- 開發流程（OpenSpec、commit 規範、驗證慣例）見 [CONTRIBUTING.md](CONTRIBUTING.md)
