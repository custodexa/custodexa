...
| `${DATA_PATH}/audit` | `/var/log/custodexa/audit` | 審計降級檔案、封存期留痕 journal | backend |
...
審計降級檔案與封存期 journal 的實際落點由環境變數 `AUDIT_LOG_PATH` 決定，
...
> - 封存期間 `/metrics` **沒有** `custodexa_audit_queue_depth`（非同步審計屬解封後才裝配的段 2），
...
- **改拓撲是一次安全變更。** 位址決定了上鎖的資料金鑰送去哪裡解，以及重包時明文金鑰材料送去哪裡。
  只有已認證的金鑰管理頁改得了它；每一次變更不論成功或被拒都以前後值寫入審計，
  成功的另經已設定的通知通道發出安全類告警（未設定通道時，審計列就是唯一的紀錄）。
  **這些都攔不住能直接寫資料庫的人**，而解封頁上的核對是一個要由人執行的步驟，
  不是系統施加的保證；那條路的防線是資料庫存取控管與離機的審計副本。
...
> 你現在看到的是較舊的目的地，須在這一頁重新設定一次；那次變更的告警與審計列
> 在被備份的那套系統裡，不在這一套。
>
