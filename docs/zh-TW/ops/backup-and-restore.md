...
稽核降級檔案與封存期 journal 的實際落點由環境變數 `AUDIT_LOG_PATH` 決定，
...
- **稽核降級檔案**：資料庫寫入失敗、或稽核佇列滿載時，稽核列改寫入此處
  （**僅在檔案降級開關啟用時**；關閉時該批紀錄直接丟棄、永久遺失）。
  它承載的是「資料庫當時寫不進去的那批稽核紀錄」，正是事故期間最需要的紀錄。
...
**資料根之外的第三格：`tls/`。** 內建 TLS 代理的憑證落在專案目錄下的 `tls/`，不在 `DATA_PATH` 之下：
`fullchain.pem` 與 `privkey.pem` 是對外憑證，自簽模式（`TLS_MODE=selfsigned`）另有簽發它們的本地 CA
（`ca-private/custodexa-ca.key` 與 `ca-public/custodexa-ca.crt`）。**這把 CA 私鑰遺失就無法沿用原 CA**，
只能改用新 CA 重簽，而換 CA 等於要把新的 CA 憑證重新派發到全部使用者端。因此 `tls/` 須納入備份範圍，
並比照 `.env` 加密保管（它同樣含私鑰）。

### 2.2 封存期留痕 journal

檔名固定為 `seal_journal.bin`，落在 `AUDIT_LOG_PATH` 所指目錄，**不另設環境變數鍵**。
內容為封存期（KEK 介面填鑰模式尚未解封時）的解封嘗試紀錄，定長環狀、永不成長。

**它可以安全納入備份，也可以安全地隨備份一起還原**：解封後 journal 會自動回灌稽核，
每則事件的冪等鍵 `IdempotencyUUID` 是由 journal UUID、序號、事件種類與槽位確定性導出的值，
且在 `audit_logs` 上有唯一索引。**同一份 journal 被重放多次不會產生重複的稽核列。**
...
- **還原到帳冊建立之前的備份**，其後上傳的遠端物件就成了**孤兒**——遠端還在（產品不刪），
  但資料庫裡沒有任何一列指得到它。要對帳時以稽核中的保管鏈事件（上傳、完整性判定、本機到期）
  比對儲存桶內的物件清單。
...
# 4. 部署層設定檔（含 KEK 材料，見第 4 節；務必加密保管）與 TLS 憑證目錄
cp "$ENV_FILE" "custodexa-env-${STAMP}.bak"
#    tls/ 含對外憑證私鑰與自簽模式的本地 CA 私鑰（見 §2.1），與 .env 同等加密保管
tar -czf "custodexa-tls-${STAMP}.tar.gz" tls

# 5. 恢復服務
#    KEK 模式 B（KEK_PROVIDER=ui）：backend 一重啟就回到**已封存**，
...
> **模式 B（`KEK_PROVIDER=ui`）：這趟備份把系統打回已封存。**
> 步驟 1 停了 backend，步驟 5 起回來的是一個**已封存**的實例——KEK 材料不落地，
> 每次啟動都要有人重新解封。備份本身不受影響（`pg_dump` 走 postgres，與封存無關），
> 受影響的是備份**之後**：在有人解封之前，登入與連線一律回 503。
>
> 兩個跟著來的後果，排例行備份時要一起想：
>
> - **排程備份等於排程一次服務中斷**，長度是「從步驟 5 到有人去解封」。半夜跑的備份多半沒人守著。
>   要嘛把備份排在有人在的時段，要嘛接受這段中斷並讓監控在已封存時告警。
> - 封存期間 `/metrics` **沒有** `custodexa_audit_queue_depth`（非同步稽核屬解封後才裝配的段 2），
...
- **不要只備份資料庫**。KEK 材料（第 4 節）、錄影檔與稽核降級檔案都不在資料庫裡。
...
備份內容含加密後的資產憑證、加密後的金鑰包裹、以及全部稽核紀錄，其敏感度不低於
...
**剪貼簿稽核內容**、**物件儲存憑證**等欄位。**資料庫備份還原後能不能解開，完全取決於 KEK（金鑰加密金鑰）能不能取回。**
**KEK 遺失＝上列全部欄位不可解**，物件儲存憑證也在其中——每一個歷史儲存世代的憑證都是這一批的一員，
解不開就取不回那個世代的離機副本。這不是離機功能帶來的新風險面，而是與剪貼簿內容、目錄整合密碼同一個命運。

> **剪貼簿稽核內容同樣依賴 KEK**：剪貼簿內容以信封加密落庫
> （`clipboard_events.content_enc`），**遺失金鑰即無法讀取剪貼簿稽核內容**——與資產憑證等其他信封加密
> 欄位同一前提，不因它是「稽核資料」而有例外。剪貼簿事件的**事實面**（時間、方向、內容長度、狀態）
...
- **材料位置**：**只在記憶體中，從不寫入磁碟**。系統啟動後處於已封存，
  除健康檢查與封存端點外全部路由回 503，須由人在 `/unseal` 頁輸入材料才解封。
...
- **一般解封**（既有部署，非初始化）：頁面先核對本地管理者的帳號與密碼，才收材料。**已封存期間不核對動態驗證碼**，因為它的種子受資料金鑰保護；這一步因此不是雙因子，紀錄上也不得這樣寫。材料本身不驗證格式
...
**只有這兩個鍵。** `KEK_KMS_REGION`、`KEK_KMS_KEY_ID`、`KEK_VAULT_ADDR`、`KEK_VAULT_ROLE_ID`、
`KEK_VAULT_SECRET_ID` 都不再生效。原本放在它們裡面的東西，現在分在兩個不同的地方：

- **非秘密的拓撲**——服務區域、保管處位址、Transit 金鑰名稱、角色識別——在金鑰管理頁設定、
  存於資料庫的 `kek_topologies` 表。**它隨資料庫備份一起走**，不必為它另外保管任何東西。
  金鑰識別本身不存在那張表裡，而是沿金鑰列的 `kek_id`，那些列同樣在資料庫裡。
- **憑證**——AWS 的 access key 組、GCP 服務帳號金鑰檔的內容、Vault 的角色密鑰或直接提供的權杖
  ——在每一次解封時於解封頁輸入，只保存在該解封世代的記憶體，封存時抹除。
  **任何備份裡都沒有它**，因為它不在任何檔案、也不在任何資料表裡。它的保管責任在部署方，
  與模式 B 的材料同一個位階。

**這對復原的影響。** 還原之後，系統知道自己的保管處在哪裡（那隨資料庫回來了），
不知道怎麼連過去（那沒有回來）。**因此還原後的第一次啟動會停在已封存等人**：
由人在解封頁以本地管理者的帳號與密碼驗證身分、拿畫面上顯示的保管處與部署紀錄核對，
再提供該服務商的憑證。等待期間行程不會退出。請把這個人與那組憑證一併寫進復原計畫；
停在「容器都起來了」就結束的還原演練，等於沒有演練過這個模式。

- **復原前提**：金鑰仍存在於保管處且仍可用、持有它的帳號或專案仍未關閉、
  且仍取得到具使用權限的憑證。金鑰被排定刪除、帳號關閉，或憑證永久取不回來，等同材料遺失。
  憑證被撤銷與金鑰被刪除不同級：憑證的簽發者可以再發一組。
- **AWS 路徑所需權限**：`kms:Encrypt`、`kms:Decrypt`、`kms:DescribeKey`；
  若使用原生重加密，另需 `kms:ReEncryptFrom` 與 `kms:ReEncryptTo` 兩個 action。
  建構時除了 `DescribeKey` 另跑一次拋棄式的加解密往返，所以缺權限會在解封當下顯現，
  而不是等到第一次實際使用。
- **憑證一律顯式注入，不自環境自動發現。** AWS SDK 預設鏈（IRSA／instance profile／`AWS_*`／SSO）
  與 GCP 的 Application Default Credentials 都不再使用，沒有提供憑證即拒絕建構。
  倚賴實例角色的還原環境不會自己起來，請在解封頁提供憑證。
- **Vault 可直接提供權杖**取代 AppRole 登入，生命週期相同：封存時抹除、到期即 fail-close。
  長壽命權杖不是建議做法——它的暴露窗口等於它的壽命，而本產品無法替它輪替。
- **信任邊界**：金鑰列的 `kek_id` 同時是信任帳號範圍的唯一來源，委託重包的目標金鑰
  必須與它屬於同一 AWS 帳號與 partition，否則拒絕。
- **改拓撲是一次安全變更。** 位址決定了上鎖的資料金鑰送去哪裡解，以及重包時明文金鑰材料送去哪裡。
  只有已認證的金鑰管理頁改得了它；每一次變更不論成功或被拒都以前後值寫入稽核，
  成功的另經已設定的通知通道發出安全類告警（未設定通道時，稽核列就是唯一的紀錄）。
  **這些都攔不住能直接寫資料庫的人**，而解封頁上的核對是一個要由人執行的步驟，
  不是系統施加的保證；那條路的防線是資料庫存取控管與離機的稽核副本。
...
| 9 | 稽核鏈驗證 | 稽核端的完整性驗證頁 | 驗證結果與備份前一致 |
| 10 | **帳冊與儲存桶內物件對得上**（僅啟用離機儲存時） | 管理端「離機儲存」頁：確認設定摘要與世代狀態如預期、失敗清單無異常堆積；抽驗一筆已離機的錄影可播 | 帳冊的狀態與遠端實況一致。**對不上的兩個方向要分開判讀**：帳冊有列而儲存桶內沒有＝還原到較新的資料庫時點（或遠端已被生命週期規則清掉）；儲存桶內有而帳冊沒有＝孤兒物件，多半是還原到較舊的時點（見 §3.1），以稽核中的保管鏈事件對帳 |

> 第 6 項是最有價值的一項：**指紋是單向摘要，比對相同即可確認還原後用的是同一把金鑰**，
> 而不必接觸任何金鑰材料。請在每次備份時一併記錄這**四個**指紋，隨備份保存。
> 備份時只記三個，還原後就有一項無從比對——漏掉的那項是檢查點簽章鑰。
>
> 金鑰管理頁要在**解封之後**才進得去（模式 B 與模式 C），故這一項排在服務已恢復之後做。
>
> **模式 C 請趁在同一頁時一併核對保管處設定**：位址、服務區域、Transit 金鑰名稱與角色識別
> 都是自備份回來的，所以它們是備份時點的樣子。若拓撲在那個時點之後被改過，
> 你現在看到的是較舊的目的地，須在這一頁重新設定一次；那次變更的告警與稽核列
> 在被備份的那套系統裡，不在這一套。
>
> 還原之後，檢查點驗證頁的角色指派比對是以備份內最新一個「存證當下比對相符」的檢查點為基準（存證時已有不符的檢查點不會被當成起算點）；還原後第一次比對相符的存證會成為新的起算點。

---

## 7. 例外情況的人工處置

### 7.1 封存期 journal 的雙 header 皆無效
...
還原出來的 `${DATA_PATH}/audit` 下若有降級檔案，代表備份時點之前曾發生過稽核寫入失敗。
