...
> （`20260816_schema_baseline`）為起點，其後以**增量 migration** 演進（本版的八條見下 §2.5）；
...
- **報告的口徑有兩處邊界，稽核讀報告前要先知道。** 一是母體只含**登記於系統的**資產帳號，
  系統不探勘目標主機；二是「共用憑證」標記只反映系統知道的複製建號關係，管理者手動編輯憑證
  不改變它，本版之前既有的複製關係也不回溯補登。兩點都寫在報告本身的口徑說明上。
...
還會有**執行期標記列**（本版為兩個：`20260804_ldap_env_seeded` 與 `20260825_offsite_env_seeded`，
分別是目錄整合與離機儲存的 env→DB 初次設定的冪等標記，**不是 migration**），
全新庫在第一次解封前沒有它們。
...
本版的資料庫有 schema baseline（`20260816_schema_baseline`）與其後的八條增量
...
- **憑證共用標示**：系統自「從其他帳號複製建號」推導出的分組。刪了即消失，再次升級之後全部
  帳號回到未分組，而且**不回溯補登**——那份關係只在建號當下記得下來。
