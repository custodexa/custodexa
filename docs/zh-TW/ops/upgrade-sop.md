...
> （`20260816_schema_baseline`）為起點，其後以**增量 migration** 演進（本版的十三條見下 §2.5）；
...
停在守衛的攔下模式：每 15 秒重取一次鎖，並只開放健康檢查、封印狀態與攔下頁的兩條端點；
攔下期間其他路徑一律回 503。
...
本版的資料庫有 schema baseline（`20260816_schema_baseline`）與其後的十三條增量
