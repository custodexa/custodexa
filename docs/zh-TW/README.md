...
- **八種協議，同一種體驗**——SSH、RDP、VNC、MySQL、PostgreSQL、SQL Server、Redis、K8s exec，
  都在瀏覽器裡開一個分頁就能連。
- **審計優先**——全協議錄影回放（可快轉、拖進度條）、指令級審計（虛擬螢幕重組，
  能正確處理 vim 這類全螢幕程式）、剪貼簿與檔案傳輸內容留痕、危險指令告警與即時阻斷、webhook 通知。
...
金鑰在你的瀏覽器本地生成（務必保存；之後每次重啟都停在封印狀態，需再輸入解封），
...
- **文字指令審計有原理性極限**（例如某些全螢幕程式的邊角行為、無回顯輸入）。
...
**單一版本，不分級。** 沒有企業版、沒有付費解鎖的功能、也沒有另行授權的模組——你看到的就是全部。
