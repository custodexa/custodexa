...
應用程式資料（稽核日誌、錄影、資料庫）的根目錄，會以 bind mount 掛進容器。
...
- **`ui`（出貨預設值）**：材料**只**存在於記憶體中——由瀏覽器在首次啟動的初始化頁面本地生成，
  ...
  代價是：**每一次**行程重啟（當機、主機重開、容器重建）回來時都是已封存狀態，
...
把它設成 `false` 會被忽略，稽核被強制開回來，而被強制的鍵名會列在啟動日誌中
（全操作稽核是一條紅線，部署層不能把它關掉）。
...
1. **至少保留一個本地管理員帳號。**
   ...
   如果所有管理員都外部化了，一套被封存的系統就再也沒有人能解封它。
...
在那之後，管理介面的「身分與權限 -> 身分來源」頁面是唯一的事實來源，
...
因此也決定了稽核日誌裡記下的位址，以及登入限速用來計數的那把鍵。
