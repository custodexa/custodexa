...
management product, and a public issue hands the attack method straight to the
...
or "patch within 7 days" would have no one to stand behind them, so writing
them down would just be an unkeepable sentence.
...
from an unauthenticated entry point?", which is exactly what CVSS base scores
capture worst. If you provide a CVSS vector we will record it, but severity is
decided on three axes:
...
"an administrator holding the signing key can re-sign", a boundary already
stated in the specifications); or an equivalent compensating control
...
own, with no further permission needed. This is **your right, not a deadline
promise from this project**: there is no guarantee a fix will exist within 90
days, but a known issue will also never be held over your head indefinitely
just because it is not fixed yet.
...
Listed here so you do not waste your time:
