...
// AssetAccount 資產帳號（asset-multi-account D1）：一資產多系統帳號，
// 各自持有加密憑證。既有 Asset 內嵌憑證由 migration 複製為一筆 IsDefault 帳號
// （密文原樣複製——信封密文自帶 DEK 版本前綴、無 AAD 列綁定，跨表可解）。
//
// default 語義（D8）：「至多一個 default」由 partial unique index
...
// 零帳號資產合法（原本即無憑證的資產）。
//
// 安全紅線（D1a）：PasswordEnc/PrivateKeyEnc 必須登記於 keyvault 的
...
type AssetAccount struct {
	...
	// AuthMethod 認證類型（mssql-web-cli D3）：sql｜domain。
