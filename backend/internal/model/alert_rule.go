...
// Alert severity 等級（command-alerts design D3）：
...
// AlertRule 危險指令告警規則（command-alerts D3/D4）
// 注意：本表由 migration v7.9 以原生 SQL 建立，不走 AutoMigrate，
// 欄位定義需與 migration 保持一致
type AlertRule struct {
	ID       uint   `gorm:"primarykey" json:"id"`
	Name     string `gorm:"size:100;not null" json:"name"`
	Pattern  string `gorm:"type:text;not null" json:"pattern"`            // regex，入庫前以 regexp.Compile 驗證（D4）
