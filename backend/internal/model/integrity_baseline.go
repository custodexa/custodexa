...
// audit-log-compliance 對抗驗證修正：Verify 原本把空 HMAC 一律歸 Legacy，
// 攻擊者竄改內容並同時清空 integrity_hmac 即可規避偵測。基準之後
// 寫入的列必帶 HMAC，空 HMAC 改判不符。
// 限制（誠實邊界）：具 DB 寫入權者可改此基準或整列刪除——與 design D6
// 一致，該層級攻擊依賴 DB 權限收斂與 SIEM 側對帳補位
type IntegrityBaseline struct {
	ID         uint      `gorm:"primarykey" json:"id"`
	BaselineAt time.Time `gorm:"not null" json:"baseline_at"`
	// MaxLogID 基準建立當下 audit_logs 最大 id（key-management-envelope
	// 對抗驗證 H2 修正）：空 HMAC 判 legacy 改以 id <= MaxLogID 為準——
