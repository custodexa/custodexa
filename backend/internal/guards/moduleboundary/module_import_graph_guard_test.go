...
//
// **為何不是「import 守衛」而是「符號參照守衛」**（本守衛的誠實邊界）：立起它的
// 當下尚未搬檔，七個模組仍同居於 `internal/service` 一個 Go package，包級 import
// 邊此刻並不存在——只看 import 會零違規恆綠，正是基線 §6.2 反覆指出的假綠形態。
...
// 現況 28 個包（拆環時新增 internal/kernel、internal/kernel/dberr）；取 24 為保守下界。
// **遷移只會增加包（internal/modules/*），不會減少**，故此下限在整段遷移期間恆有效。
const minModuleGraphPackages = 24

// minAttributedServiceFiles 落入模組歸屬的非測試檔數下限。
// 現況 85 檔（完全劃分的 84 檔＋另行拆出的 keyvault/release.go）；取 80 為下界——
// 掉到下限以下代表掃描範圍或歸屬判定失真，此時「零違規」不成立。
const minAttributedServiceFiles = 80

// serviceScanDir 已消滅的扁平 service 包（相對 module 根，slash 分隔）。
// **它必須不存在**——七模組全數搬包完成，`internal/service` 連同其
...
// moduleRootPrefix 已搬包模組的路徑前綴。
const moduleRootPrefix = "internal/modules/"

// 要釘住的禁止邊。key＝依賴方模組，value＝其不得參照的模組集合。
//
// **起初只列三條，不多不少**：1.10–1.12 就地拆掉的三條真環（4.9／4.10／4.11）。
// 其餘矩陣邊（policy→authz 屬 §4.8；模組對 `internal/repository` 的直讀等）
// 當時尚未拆解，列入只會得到一份必紅的清單，反而逼人放寬守衛——那正是驗收條件
// 明令禁止的方向。每拆掉一條 SHALL 在此加行，並附拆解依據。
var forbiddenModuleEdges = map[string]map[string]bool{
	...
	//
	// **policy 列改為全零出向**（3.8 驗收條件）：§4.8 環拆解後
	// （`AnnotateConnectStates` 歸 authz、`ConnectSourceResolver` 窄介面反轉），
	// policy 對六個業務模組皆無出向。起初只釘 policy→audit，是因為當時 §4.8 尚未拆；
	// 現況已可釘滿整列（依賴矩陣的 policy 列全 ✗）。
	"policy": {
		"audit":    true,
		"authz":    true,
		"identity": true,
	...
	// **新增 audit 列**（4.13 驗收條件）：15 檔搬入 `internal/modules/audit` 後，
	...
	// 兩者在依賴矩陣中即為 ✔。
	//
	// 同時拆掉的三條真出向邊：`RecordingService`（session）於 4.8 反轉為
	...
	// **新增 asset 列**（6.7 驗收條件）：9 檔搬入 `internal/modules/asset` 後，
	// asset 對 authz／identity／session 零出向。**刻意不列 audit／keyvault／policy**
	// ——那三條是依賴矩陣中既有的合法出向邊：交易內審計走
	// `audit/port.TxSink`、憑證欄的 CipherRef 常數來自 keyvault、資產列表的傳輸風險
	// 徽章來自 policy。
	//
	// 同時拆掉的兩條真出向邊：`FillNodeInfoForDTOs`（吃 authz 的 AuthorizedAssetDTO）
	// 整個方法改掛 `*AssetAuthorizationService`（§5.5，提前執行 7.6）；
	...
	// **新增 authz 列**（7.10 驗收條件）：9＋5 檔搬入 `internal/modules/authz` 後，
	// authz 對 identity／session／keyvault 零出向。**刻意不列 asset／audit／policy**
	// ——那三條是依賴矩陣中既有的合法出向邊：`asset.ValidateAccountUsername`／
	// `asset.FillNodeInfo`、交易內審計走 `audit/port.TxSink` 與 `audit.AuditLogService`、
	// 存取政策與時長上限來自 policy。
	//
	// 同時拆掉的唯一一條真出向邊：`AccessRequestService.sessions` 的型別由
	...
	// **新增 identity 列**（9.10 驗收條件）：31 檔搬入 `internal/modules/identity`
	// 後，identity 對 asset／session 零出向。**刻意不列 audit／authz／keyvault／policy**
	// ——那四條是依賴矩陣中既有的合法出向邊：外部身分與 OIDC/LDAP 的審計走
	...
	//
	// **零真出向邊要拆**（開工實測）：identity 對 asset 與 session 的跨模組符號
...
// **不再有「尚未搬包」的檔**：原本此處有一份 `serviceFileModule` 歸屬表，
// 登記 `internal/service` 中尚未搬包之非測試檔的模組歸屬。七個模組搬完後該包已解散
// （最後搬走 session 的 8 個非測試檔，其餘 13 個測試檔改居擁有者模組的外部測試包），
...
// TempDir 造樣本樹。故把判定與掃描切開，餵合成的違規表
...
//
// **只剩一條判準**＝`internal/modules/<name>/` 路徑前綴。遷移期間另有一條
// 「查 `internal/service` 歸屬表」的分支服務尚未搬包的檔，該包已解散。
...
func TestModuleGraphHasNoForbiddenEdges(t *testing.T) {
	...
	t.Fatalf("偵測到 %d 筆禁止的跨模組參照（1.10–1.12 拆掉的環不得回潮）：\n%s",
