// Package txtaking 承載「跨模組 tx-taking 呼叫白名單」機械守衛。
//
...
// 它的兩支測試各自重跑一次全樹型別檢查，合計耗時與 moduleboundary 其餘各支同級，
// 獨立出來可讓兩邊各自遠離 600 秒上限。
//
...
// （gwModuleRoot／guardModuleRoot／auditPointModuleRoot／gateModuleRoot），
// 此處延續同一慣例。
