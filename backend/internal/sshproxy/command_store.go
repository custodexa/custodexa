...
type CommandStore struct {
	...
	// K8s 當次 pod/container（k8s-exec D5）：冗餘入每條指令，免 JOIN 跨會話搜尋
	k8sPod       string
	k8sContainer string
	...
	// alerts 降級告警的落地面（design §6.3）：與阻斷路徑同一個 AlertSink，
...
// SetAlertSink 掛上降級告警的落地面（design §6.3）。
//
...
// SetK8s 設定當次 K8s pod/container（k8s-exec D5：冗餘入每條指令）
...
//
// **共用 s.seq++**（design §6.4）：seq 是會話內的執行順序，兩個讀取面都靠它排序
	...
	flush := func() {
		...
		if err := s.db.CreateInBatches(batch, commandWriteBatchSize).Error; err != nil {
			...
			if matcher := audit.GetAlertMatcher(); matcher != nil {
				// 告警比對掛載點（command-alerts D2）：flush 成功後逐條比對，
				// MatchAndStore 內部錯誤僅 log，不反向影響入庫
				matcher.MatchAndStore(batch, s.protocol)
			}
			// 降級告警的專用發射器（design §6.3）：同樣在**入庫成功之後**，
...
//
// **降級告警的 drain 點即此**（design §6.3）：會話在降級中結束時，最後一批降級列
