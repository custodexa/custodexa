...
//
// **W7b 8.3**：舊的 `ApproverAdminKey`（審核端點的 admin 兜底旗標）隨 D-12 收斂
// 移除——admin 不再是有效審核者，審核端點不存在 admin 兜底身分。
const RevokeAdminKey = "revokeIsAdmin"

// RequireApproverRole 審核端點守門（access-policy-approval D5；
// approval-routing-quorum D-7 擴群組即資格）：即時查 DB，
...
//
// **D-12 行為變更（W7b 8.1，BREAKING）**：移除 `isAdmin` 放行分支——
...
// 故零有效審核者時 admin 仍能指派 approver 使系統復原（D-12 硬性附帶條件）。
...
// RequireRevokeEligibility 撤銷端點守門（W7b 8.2 端點分離）：
...
// （design.md D-12 訂正）。細緻資格由 service 的 `eligibleToRevoke` 裁決。
