...
// TestRequireApproverRole 守門即時查 DB roles：
// approver 放行、一般 user 403、撤 approver 即刻生效（無 token 殘窗）。
//
// **行為變更**：僅具 `admin` 角色者**不再放行**（原「admin 兜底放行」
...
func TestRequireApproverRole(t *testing.T) {
	...
	t.Run("approver 放行", func(t *testing.T) {
		...
		if _, ok := keys[RevokeAdminKey]; ok {
			t.Fatal("審核端點守衛不應寫入任何 admin 旗標")
		}
	})

	// 行為變更（BREAKING）：僅具 admin 角色者不再是有效審核者
	t.Run("僅具 admin 者 403", func(t *testing.T) {
		code, _ := callGuard(db, 3)
		if code != http.StatusForbidden {
			t.Fatalf("僅具 admin 者應 403（admin 不構成審核資格）, got %d", code)
		}
	})
	...
	t.Run("審核方群組成員即資格（群組即資格）", func(t *testing.T) {
...
// TestRequireRevokeEligibility 撤銷端點守門（與審核端點分離）：判準＝
