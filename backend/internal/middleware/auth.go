...
// authRejectionContextKey 認證中介層拒絕標記（audit-coverage-closure D1）。
//
...
// 開出一個憑證存在性的探測面（同批 4 對 /connect 拒絕原因的處置）
...
// JWT 僅從 Authorization header 接受（transmission-security-policy M8）：
...
func AuthMiddleware(authService *identity.AuthService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// scoped token deny-by-default（auth-hardening D4）：任何帶 scope 的中繼 token
		...
		// 憑證世代閘（idp-oidc-integration D2/D9）：provider 停用/刪除/密鑰輪替，
		...
		// 認證脈絡（idp-oidc-integration D9）：connect token 簽發點須據此填入 grant，
...
// GetAuthContext 從 gin context 取得本次請求的認證脈絡（idp-oidc-integration 1.9）。
//
