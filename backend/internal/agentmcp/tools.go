...
var descriptions = map[string]string{
	...
	"check_request":  "用 request_access 的 request_id 查看 created_at、status、approvals_received、approvals_required、expires_at。pending 且未到期並有核准進度時繼續等；仍未足額時改做別的事並稍後再查；rejected、revoked 或 expired 時判定不會過，停止等待。",
	"open_session":   "用 list_assets 的 asset_id、account_id 與 request_access 的 request_id 建立有錄影的 SSH／K8s 終端或資料庫查詢會話。request_id 必填，不會替你挑選任務。回傳 session_handle 只限本 MCP 連線及 token 使用；拒絕時依機器码修正任務或等待核准，不繞過閘序。K8s 必須指定 pod，可選 container 與 exec/logs 模式。",
	...
	"read_screen":    "用 session_handle 取得累積輸出去控制序列後最後 N 行。這是行緩衝，非虛擬螢幕；畫面清除、游標定位、全螢幕程式下與真實終端不一致，不能視為畫面快照。失效句柄須重建經核准的會話。",
	"close_session":  "關閉 open_session 所發的 session_handle 並結算錄影與指令審計。已終止時停止使用句柄；不存在時不猜測其他句柄。關閉只終止本系統建立的會話，不保證目標端背景程序停止。",
	"close_task":     "用 request_id 與 report 關閉自己的任務並提交報告版本；再次呼叫新增版本、不覆写舊版，限原提交主體及持久化關閉時刻起 24 小時內，逾窗不重試。close_task 不是關閉的唯一途徑：全部任務項到期或被撤銷也會關閉並終止其餘會話。缺報告以關閉時刻判定，其後句柄回已終止狀態。",
}
