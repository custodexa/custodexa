...
//
//	MFA 能力  auth 以 NewAuthServiceWithMFA 建構——沒有 mfaCrypto 就簽不出可完成的
...
func (e *oidcLifecycleEnv) oidcCtxFor(user *model.User) crypto.AuthContext {
	return e.auth.buildAuthContext(user, crypto.AuthMethodOIDC, e.provider.ID)
}
...
func (e *oidcLifecycleEnv) setProviderEnabled(t *testing.T, enabled bool) {
	t.Helper()
	if _, err := e.providers.Update(e.provider.ID, &OIDCProviderRequest{
...
func beginFlowWithChallenge(t *testing.T, login *OIDCLoginService, p *model.OIDCProvider,
