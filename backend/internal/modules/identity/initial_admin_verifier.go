...
// 初始管理員憑證驗證（kek-provider-modularization D6.3 初始化解封專用）。
//
// **為何住在 identity 而不是 keyvault**（modular-architecture W6 6.0d，源自
// codex W2 外審 K4／W2 對抗 M3）：本函式做的三件事——查 `users`、展開 `Roles`
...
// 並在包內執行 identity 的認證判定（backlog B-13）。搬到 identity 側後，
...
// D6.6 的 MFA 死鎖——該死鎖論證只對既有部署的一般解封成立。
...
func VerifyInitialAdminCredential(db *gorm.DB, username string, password []byte) error {
	...
	//
	// 判定用 IsExternal() 而非 IsLDAP（idp-oidc-integration D8）：OIDC 影子帳號的
