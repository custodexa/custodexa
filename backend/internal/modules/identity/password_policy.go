...
// 的 `password_compliance.go`，modular-architecture W3 3.3／R3.1 §5.2）。
//
...
//
// **不再寫死 72**（password-hasher-interface 5.2／5.3）：那是 bcrypt 特有的限制，
...
// ValidateNewPassword service 層單一密碼 validator（D3，政策驅動）：
...
func (s *UserService) ValidateNewPassword(userID uint, newPassword string) error {
	...
	// 輸入上限先擋（PW-2）：超過會讓雜湊實作回錯而落到 500，改回人話 400。
...
// CheckPasswordCompliance 無狀態政策合規子集（login-password-policy-gate D2）。
...
// 每次設定密碼都會寫入歷史（含初始密碼，D12），故近 N 筆已涵蓋現行密碼
func (s *UserService) isPasswordReused(userID uint, newPassword string, historyCount int) (bool, error) {
	...
	//
	// **密碼歷史 SHALL NOT 被重新雜湊**（password-hasher-interface 3.2）：
...
func (s *UserService) recordPasswordHistory(tx *gorm.DB, userID uint, passwordHash string) error {
	...
	// 保留最近 keep 筆，刪其餘。以單調遞增主鍵 id 排序與刪除（PW-5）：
