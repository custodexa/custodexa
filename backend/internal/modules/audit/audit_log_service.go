...
func safeAuditSubstanceFields() map[string]bool {
	return map[string]bool{
		...
		// 與「固定成一般使用者角色」在 request_body 面寫出同一列
		"role": true,
		// `match_value`：一條映射規則比對的外部群組值（目錄側為群組的辨識名稱、
		// 提供者側為宣告裡的字面值）。它與 role 合起來才是這條規則的內容本身——
		...
		// ── 憑證庫：範圍、改密模式、新秘密來源與掛載的新憑證 ───────────
		// 四者都是枚舉或自增 id，且都是「動成什麼」——少了它們，
		// 「把一組秘密改成全站共用」與「改個備註」寫出同一列。
		//
		// scope：專用↔共用。共用即代表這組秘密的影響面從一台擴到 N 台，
		// 那是本產品最需要被看見的一種變更。
		// mode：整組同一組秘密，或每台各自隨機並就此解除共用——兩者對
		// 「哪幾台之後還是同一組」的結論相反。
		// source：新秘密由系統隨機產生，或由操作者指定（秘密**本體**仍遮）。
		//
		// **`credential_id` 刻意不在此**：它命中 G3 的機密語義片段（credential），
		// 而 G3 的過度攔截是刻意的安全側，不為個案開名稱例外（同 `key_strategy`
		// 一族的既有裁決）。更換掛載憑證的課責改由憑證服務自寫的專屬審計列承擔。
		//
		// **這三個鍵的射程大於登記理由**：本清單按鍵名全域比對（遮罩看不到請求打的是
		// 哪個端點，見上方 `url` 的同一條款），故其他端點上同名的欄位一併被放行。
		// 已知的同名處是存取覆核的範圍欄、匯出與檔案傳輸的模式欄，以及來源政策、
		// 錄影與資產授權的來源欄——逐一看過都是枚舉或短識別字，不承載機密，
		// 故維持登記。要把射程收到單一端點，前置條件與 `url` 同一個：端點感知遮罩
		"scope":  true,
		"mode":   true,
		"source": true,
