...
// 自 transmission_inventory_service_test.go 拆出（該檔已改為 policy_test）。

// TestLegacyDetailCollisionConserved detail_codes→legacy 顯示鍵碰撞時 count 守恆：
// 髒資料使 unset 與字面 "(未設定)" 同時存在，兩者映到同一 zh 鍵須累加不覆蓋。
