...
// 方向即翻轉為 audit→policy 單向（相依矩陣：policy 列全 ✗）。
//
// 回傳型別是 `model.NotificationChannel`：`internal/model` 為共用包（已裁決不拆），
// policy 與 audit 皆合法依賴它，故用它承載清冊列不會把邊加回來。
// **偏離旗標 `TransmissionDeviation` 由 audit 於 `toDisplay` 填妥**——此處不搬動
...
// TransmissionInventoryService 通道加密清冊：
// 讀時彙整不落表。資產類以 SQL group by 聚合（不逐列），每個欄位組合以
// 代表資產映射回 TransmissionPolicyService 判定——count 在 SQL、規則在判定
// 單一所在，聚合不會與閘門漂移
...
type InventoryChannel struct {
	...
	// PreflightCode/PreflightParams 前端 i18n（count 為整數）
...
func init() {
	// note（9）：LDAP 三完整碼（啟用／未啟用／讀取失敗），不用中文前綴組合
...
func (s *TransmissionInventoryService) buildDB() (InventoryChannel, error) {
	...
	for _, r := range rows {
		// tls mode enum 為語言中性技術鍵；未設定改機器鍵 unset
		mode := r.DBTLSMode
		if mode == "" {
...
func (s *TransmissionInventoryService) buildSyslog() (InventoryChannel, error) {
	...
	if len(ch.Risks) > 0 {
		ch.AtRiskCount = 1
		// 供清冊頁 riskLabel 查譯 syslog_non_tls 的 {protocol}
