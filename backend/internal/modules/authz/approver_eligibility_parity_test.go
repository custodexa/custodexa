...
//
// **收斂後兩個入口必須逐例一致，含 admin 兩類。** 收斂前守衛入口
// 另有一份含 admin 放行的 SQL，本測試曾以 `mustAgree=false` 把該落差釘成機器
// 事實；落差消滅之後六類 fixture 一律要求一致，且「僅具 admin」的期望值
// 由 true 改為 **false**（資格改由 approver 角色／審核方群組決定，非 admin 身分）。
//
// **守衛的現實失效模式**：有人在任一入口重新加回 admin 放行分支（例如為了讓
// admin「看得到待審清單」而順手改回），此測試立刻轉紅。
//
// 撤銷端點**不在**本等價關係內——`EvaluateRevokeRouteEligibility` 刻意保留
// admin 放行（遏制動作非審核），其行為由
...
func TestApproverEligibilityParity(t *testing.T) {
	...
	cases := []struct {
		name string
		id   uint
		// wantGuard 審核端點守衛入口的期望（不含 admin）
		wantGuard bool
	...
	for _, c := range cases {
		...
		if verdict.Allowed != effective {
			t.Errorf("%s：兩個入口**必須**逐例一致卻分歧（守衛=%v／有效審核者=%v）——"+
				"收斂後審核資格只有一份真相，分歧代表有人在某一側加回了獨立判準",
				c.name, verdict.Allowed, effective)
		}
	}
}

// TestRevokeRouteEligibilityKeepsAdmin 撤銷端點守衛**刻意不收斂**（端點分離）：
// 撤銷是遏制動作不是審核，既有 spec 明定資格＝admin OR 原核准人；一併收斂會使
// admin 無法撤銷已核出的票證＝安全倒退
