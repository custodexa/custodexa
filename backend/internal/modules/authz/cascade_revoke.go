...
// F8 交易級聯撤銷的 tx-taking 窄 port（D-10 拍板形態，modular-architecture W7 7.4）。
//
// **問題**：刪除資產組／使用者群組／使用者時，必須在**同一交易**內連動撤銷 authz 的
// 授權列與審核範圍（不留幽靈引用，對抗驗證 aaa2018）。呼叫方（asset／identity）
...
// (c) code review。**白名單有 commit 權者可刪**，這是設計上接受的殘餘風險（D-10）。
//
...
func (s *AssetAuthorizationService) RevokeByAssetGroup(tx *gorm.DB, groupID uint) (int64, int64, error) {
	...
	// 連動軟刪 approver 審核範圍（D4：範圍與授權客體同構，懸掛範圍同樣誤導）
...
// RevokeByAsset 資產刪除時的級聯撤銷（security-backlog-settlement 塊 2）。
...
// 範圍——與 RevokeByAssetGroup 處理 `asset_group_id` 的理由相同（對抗驗證 aaa2018）。
//
...
func (s *AssetAuthorizationService) RevokeByUserGroup(tx *gorm.DB, groupID uint) (int64, error) {
	...
	// 連動軟刪審核範圍（approval-routing-quorum D-7）：群組作審核方
