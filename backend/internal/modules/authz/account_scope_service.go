...
//
// 強制點三處：connect token 簽發、兌換複查、帳號列表／選擇器過濾；
...
// ErrAccountNotAuthorized 帳號不在使用者的有效授權範圍內。
//
...
//
// k8s 資產豁免：固定單一預設帳號的系統語義，不經 per-account 判定。
...
// **拆檔**：`AccountIdentity` 與 `(*AssetService).ResolveAccountIdentity`
// 已遷入 `asset_account_identity.go`——它們是 asset 的型別與方法（「型別的方法必須與
// 型別同包」），留在本檔會讓 authz 的檔案在搬包時把 asset 的方法一起帶走。
