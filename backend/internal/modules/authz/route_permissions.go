...
// 路由級 RBAC 表（SD-4 第一步，modular-architecture W7 7.2）。
//
// 本檔自 `internal/middleware/permission.go` **原樣搬入**：權限常數值、
// 角色→權限對照、判定順序逐字未改，行為位元相同。搬入的理由是 SD-4 指出的
// 「權限判定第三份真相散落在 middleware」——表與判定函式屬授權語義，
// 應與資產級 ACL 同住 authz，middleware 只保留「取角色 → 問 authz → 回 403」
// 這段 HTTP 關注點。
//
// **刻意不做的事**：與 `AssetAuthorizationService` 的權限階層合流是 SD-4 第二步，
// 屬路由級 RBAC 與資產級 ACL 的語義合流，是設計變更不是重構，依 D-8 延後
// （backlog B-3）。本檔不得引入任何 DB 存取或模組相依——它是純表＋純函式。
...
func RoutePermissions(role string, perm Permission) bool {
	...
	// 定義 user 角色權限（最小權限，7.2.x/D9 + session-access-scoping）：
	...
	// 定義 auditor 角色權限：顯式保留 audit/alert view（D9：不可用 append(userPermissions)
