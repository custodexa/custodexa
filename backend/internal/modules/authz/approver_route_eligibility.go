...
//
// 等價比對見 `approver_eligibility_parity_test.go`（收斂後改為
// 斷言兩實作**逐例一致**，含 admin 兩類皆不放行）。

// ApproverRouteEligibility 審核端點守門的判定結果。
//
// `IsAdmin` 欄位已移除——admin 兜底語義隨收斂消滅，
...
// EvaluateApproverRouteEligibility 即時查 DB 判定審核端點資格（不含 admin）。
...
// EvaluateRevokeRouteEligibility 撤銷端點守門（端點分離）：admin OR 有效審核者。
//
