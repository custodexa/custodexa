...
// db 可為 *gorm.DB 或鎖內交易——鎖內判定一律傳 tx（判定以鎖內重讀為準）
...
//
// **存在理由是 4.10 環拆解**：KEKRetirementMonitor 原本
...
// KEKRetirementMonitor 退役收斂 degraded 評估器。
...
type KEKRetirementMonitor struct {
	...
	//
	// 語義是「已提醒」而非「已評估」：政策關閉時 sendNotify
...
func (m *KEKRetirementMonitor) Evaluate(now time.Time) {
	...
	if count > 0 {
		...
		if remind {
			...
			// （遷移前文案即為「N 筆舊 KEK 包裹列仍未退役」，此處補回）。
			// 收尾錯誤原文等 forensic 明細仍只留 cause_params，不進出站 payload
...
// detail 屬 forensic，落 cause_params 與本地 log，不進出站 payload
