...
// 換鑰精靈（key-management-envelope D5）：全部操作皆管理員手動觸發，
// 系統永不自動輪換任何金鑰。

// rotationMaxPerRunDefault 單次重加密上限預設（沿 retention 慣例）
const rotationMaxPerRunDefault = 100000

// rotationMinPerRun 單次上限的下界（policy-numeric-lower-bounds）：
...
// ErrRewrapPendingExists 已有待切換 pending 時拒絕新重包（key-inventory-transparency）：
...
// ErrRetireBacklog 前次切換未成功退役的舊列尚未收斂時拒絕新重包（key-inventory-transparency）
var ErrRetireBacklog = errors.New("前次 KEK 切換的舊列尚未完成退役收斂：請先重啟後端讓其自動收斂，再開始新重包")

// ErrStaleKeyCache 本實例 in-memory 金鑰快取與 DB 現行版本不符（codex 第一輪審 #3）：
...
// activeVersionTx 鎖內讀 DB 現行 active 版本（env live 代表列；D3——判定以鎖內重讀為準）。
// 查無列＝本實例 KEK 已無 live 代表（另一實例已完成切換、本實例 env 已被退役，
// opus 第一輪審 CRITICAL 的實測情境）——此時絕不可放行輪替，否則會鑄出僅被
...
// data_keys 判定與寫入在跨實例互斥鎖內（D3：campaign 判定 DB 現查，不憑
...
// 不重算歷史章（D4 版本化的意義）——新列以新鑰蓋章，歷史列以其 key_version 驗證。
// data_keys 判定與寫入在跨實例互斥鎖內（D3）。
...
// rejectPendingCampaignTx campaign 判定（D3 鎖內 DB 現查）：存在任何待切換
...
// KEKRewrapResult KEK 重包結果（kek-provider-modularization D7 明文流向反轉後）。
//
...
//
// **回應形狀由 union 判別（D7／D11.1 裁決 6）**：TargetMode 是判別子；本地目標的
...
func (s *KeyManagerService) RewrapKEK(ctx context.Context, target *RewrapTarget) (*KEKRewrapResult, error) {
	...
	// **sink 端重驗不變式**（codex R2）：欄位不導出只擋得住套件外的呼叫端，
	...
	err := s.withDataKeysLock(func(tx *gorm.DB) error {
		...
		// 不靜默清除既有 pending 而使已交付的新 KEK 失效，key-inventory-transparency）
		var pendingCount int64
		...
		// 守衛 (b2)：目標不得等於現行 KEK（D8「非現行 KEK」）。守衛 (c) 亦會擋下
		...
		//
		// **本地與委託的判定範圍刻意不同（D11.1 裁決 6，opus H4）**：
		...
		// 且 partial 唯一索引仍擋住未退役列並存。此取捨已於 design D11.1 裁決 6 明載。
		...
		for _, row := range rows {
			// 已清理佔位（D9）：材料已銷毀，佔位隨行複製至新 KEK 保版本鏈不斷號
	...
	s.mu.Unlock()
	// 日誌只帶金鑰引用（非機密），**永不含材料**——D7 的「不落日誌」落點之一
...
// 現行 KEK 指紋者）**軟退役**（reason=abandoned、材料保留至顯式清理——D9 軟刪除
