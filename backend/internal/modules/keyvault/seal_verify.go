...
// B 模式解封的材料驗證原語。
//
// 三個函式都在**臨界區之內**被呼叫（CAS 取得持有權在任何驗證之前），
...
// `VerifyInitialAdminCredential` 與 `ErrSealInitialAdminInvalid`
...
// 在資料層直接讀 identity 的三張表。

// CountDataKeys 回傳金鑰表筆數。
//
// **沿用 InitKeyManager 既有的 count == 0 判定**，不另造判準：
...
//
// 這是一般解封的唯一判準：金鑰引用一致**且**現行代表列全數實際
