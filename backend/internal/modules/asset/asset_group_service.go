...
// treeStructMu 樹結構變更互斥（asset-node-tree codex 對抗審查 P1）：
...
// 實例，design D7）以進程內互斥消除 TOCTOU——管理操作低頻，鎖成本可忽略
...
// maxNodeDepth 樹深上限（asset-node-tree D1：防失控巢狀）
const maxNodeDepth = 10

// AssetGroupRequest 節點建立/更新請求（asset-node-tree：分組升級節點樹；
...
// AssetGroupService 資產節點樹 CRUD（asset-node-tree）
type AssetGroupService struct {
	db *gorm.DB
	// auditTx 交易內審計落地面（W4 4.4）：節點結構變更與刪除的留痕與變更同交易，
	// 留痕失敗即回滾（授權涵蓋範圍變更不可無痕）。未注入時寫入回 error，
	// 語義與「審計寫失敗」同格——見 internal/modules/audit/port/txsink.go 的 port.WriteInTx
	auditTx port.TxSink
	// authzRevoker 節點刪除時的 authz 級聯撤銷（F8／D-10 tx-taking 窄 port）
	authzRevoker assetGroupAuthorizationRevoker
}

// assetGroupAuthorizationRevoker 節點刪除時對 authz 表的級聯撤銷（F8）。
//
// **消費者側窄介面（D-10）**：asset 不得 import authz（矩陣 asset→authz ✗），
...
// ListWithAssets 全節點＋直掛資產（asset-node-tree：成員經 asset_nodes M2M）
...
// TreeNode 樹導覽節點 DTO（asset-node-tree D5）
...
// TreeVisibility 樹收斂範圍（asset-node-tree D6）：nil＝全量（admin/auditor）。
// NodeIDs＝可視節點鏈；AssetIDs＝可視資產集——計數與 has_children 同受收斂
// （codex 對抗審查 P1：僅過濾節點不過濾計數，會經 subtree_asset_count 洩漏
// 隱藏子樹的未授權資產數量與結構存在性）
...
// Tree 樹導覽（asset-node-tree D5）：parentID nil＝根層，非 nil＝該節點子層
...
// isDescendantTx 判 candidate 是否為 root 的子孫（含自身）——環路檢查（D4）
...
// nodeAudit 節點結構變更審計（codex 對抗審查 P2：搬移/建立/改名即授權
// 涵蓋範圍變更，須留痕可追溯）；與變更同交易——審計失敗即回滾。
//
// **W4 4.4 收口**：原為自由函式直寫 `tx.Create(&model.AuditLog{...})`，改為經
...
func (s *AssetGroupService) nodeAudit(tx *gorm.DB, action model.AuditAction, nodeID uint, actorID uint, actorName, clientIP, details string) error {
	// **AssetID 刻意留空**（auditor-workbench D4）：ResourceID 是**節點 id**，不是資產 id。
	// 節點建立／改名／搬移一次影響其下全部資產，沒有單一主體可釘；把 nodeID 填進
	// asset_id 等於宣稱「這件事發生在編號相同的那台資產上」——那正是 D1.3(a) 要消滅的假事件。
...
// Move 搬移節點（asset-node-tree D4）：環路檢查（目標不得在自身子樹內）、
// 深度重驗（搬移後整棵子樹不得超限）、同層同名 409；treeStructMu 消除
// 並發互搬建環/超深窗口（codex P1）。搬移即子樹授權涵蓋變更，入審計。
...
// Delete 刪除節點（asset-node-tree D4）：僅「無子節點且無直掛資產」可刪
// （防誤刪大子樹）；掛該節點的授權與 approver 審核範圍
// 連動軟刪＋審計留痕同交易。回傳連動撤銷的授權筆數。
// treeStructMu 消除「空節點判定後並發掛載/建子」窗口（codex P1）
func (s *AssetGroupService) Delete(id uint, actorID uint, actorName, clientIP string) (int64, error) {
	...
	err := s.db.Transaction(func(tx *gorm.DB) error {
		...
		// 連動軟刪節點授權與 approver 審核範圍（F8／D-10）：兩張表皆屬 authz，
		...
		groupID := id
		// W4 4.4 收口（AP-37）：留痕與級聯撤銷同交易，寫不進去即整筆回滾
