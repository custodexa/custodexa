...
// 撤銷管道接線的**執行期** oracle（backlog B-50：同源 oracle 盲區）。
//
...
// 這是安全紅線（授權繞過），故依 B-50 判準補一條與名稱無關的軸。
//
...
// 對象」（接到一個非 nil 的 no-op 樁仍會綠）。後者屬 B-51（登記項語義漂移）
// 的範圍，不在本格射程。行為面由各模組既有的撤銷矩陣測試涵蓋——但那些測試
