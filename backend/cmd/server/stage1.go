...
func runStage1() *stage1 {
	...
	// release 安全底線：把安全紅線類 feature flag 強制回啟用。
	//
	...
	// 判定不經 config 的預設值注入：金鑰類鍵一律 os.LookupEnv 三值語義。
	...
	// 生產部署基線自檢（PCI 2.2.2/7.2.x）：release 模式 fail-close
	...
	if userCount == 0 {
		...
		// 既有安裝（階段 5）：初始密碼已退役。仍留有效格式值 → 告警提醒移除/輪替，不 fail-close
	...
	// 啟動期清掃殘留臨時 kubeconfig（k8s-exec：補後端崩潰/OOM/SIGKILL 未跑 Close 的路徑）
	if n := k8sproxy.SweepResidualKubeconfigs(); n > 0 {
		log.Printf("啟動清掃：移除 %d 個殘留 k8sproxy 臨時目錄", n)
	}

	// CORS（PCI 7.3）：於此計算設定並**建構 middleware**——cors.New 會 Validate 並在
