...
var auditRouteRegistry = map[[2]string]routeAuditEntry{
	...
	{"GET", "/api/v1/change-secret-batches"}:                                         {classResource, model.ResourceChangeSecretPlan, "命中分類器段 `change-secret-batches`；併入改密計畫分類（varchar(20) 放不下獨立常數）"},
	{"POST", "/api/v1/change-secret-batches"}:                                        {classResource, model.ResourceChangeSecretPlan, "命中分類器段 `change-secret-batches`；併入改密計畫分類；一次作用於多台，不注入資產主體"},
	{"GET", "/api/v1/change-secret-batches/:id"}:                                     {classResource, model.ResourceChangeSecretPlan, "命中分類器段 `change-secret-batches`；`:id` 指向批次列，非資產 id"},
	{"GET", "/api/v1/change-secret-batches/targets"}:                                 {classResource, model.ResourceChangeSecretPlan, "命中分類器段 `change-secret-batches`；併入改密計畫分類（varchar(20) 放不下獨立常數）"},
	{"GET", "/api/v1/change-secret-batches/usernames"}:                               {classResource, model.ResourceChangeSecretPlan, "命中分類器段 `change-secret-batches`；併入改密計畫分類（varchar(20) 放不下獨立常數）"},
	...
	{"GET", "/api/v1/credentials"}:                                                   {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列、`:accountId`／`:rid`／`:mid` 指向掛載列與改密輪次成員，**都不是資產 id**；另入 auditSensitiveResources（讀取的是「哪些主機共用同一組秘密」的分布圖，須記查詢條件摘要）"},
	{"POST", "/api/v1/credentials"}:                                                  {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列，**不是資產 id**；掛載類動作的受影響資產由 handler 顯式注入主體鍵（路由層推導不出那台機器是誰）"},
	{"GET", "/api/v1/credentials/:id"}:                                               {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列、`:accountId`／`:rid`／`:mid` 指向掛載列與改密輪次成員，**都不是資產 id**；另入 auditSensitiveResources（讀取的是「哪些主機共用同一組秘密」的分布圖，須記查詢條件摘要）"},
	{"PUT", "/api/v1/credentials/:id"}:                                               {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列，**不是資產 id**；掛載類動作的受影響資產由 handler 顯式注入主體鍵（路由層推導不出那台機器是誰）"},
	{"DELETE", "/api/v1/credentials/:id"}:                                            {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列，**不是資產 id**；掛載類動作的受影響資產由 handler 顯式注入主體鍵（路由層推導不出那台機器是誰）"},
	{"POST", "/api/v1/credentials/:id/bindings"}:                                     {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列，**不是資產 id**；掛載類動作的受影響資產由 handler 顯式注入主體鍵（路由層推導不出那台機器是誰）"},
	{"DELETE", "/api/v1/credentials/:id/bindings/:accountId"}:                        {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列，**不是資產 id**；掛載類動作的受影響資產由 handler 顯式注入主體鍵（路由層推導不出那台機器是誰）"},
	{"POST", "/api/v1/credentials/:id/bindings/:accountId/detach"}:                   {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列，**不是資產 id**；掛載類動作的受影響資產由 handler 顯式注入主體鍵（路由層推導不出那台機器是誰）"},
	{"POST", "/api/v1/credentials/:id/scope"}:                                        {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列，**不是資產 id**；掛載類動作的受影響資產由 handler 顯式注入主體鍵（路由層推導不出那台機器是誰）"},
	{"POST", "/api/v1/credentials/:id/secret"}:                                       {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列，**不是資產 id**；掛載類動作的受影響資產由 handler 顯式注入主體鍵（路由層推導不出那台機器是誰）"},
	{"POST", "/api/v1/credentials/:id/rotations"}:                                    {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列，**不是資產 id**；掛載類動作的受影響資產由 handler 顯式注入主體鍵（路由層推導不出那台機器是誰）"},
	{"GET", "/api/v1/credentials/:id/rotations/:rid"}:                                {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列、`:accountId`／`:rid`／`:mid` 指向掛載列與改密輪次成員，**都不是資產 id**；另入 auditSensitiveResources（讀取的是「哪些主機共用同一組秘密」的分布圖，須記查詢條件摘要）"},
	{"POST", "/api/v1/credentials/:id/rotations/:rid/members/:mid/retry"}:            {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列，**不是資產 id**；掛載類動作的受影響資產由 handler 顯式注入主體鍵（路由層推導不出那台機器是誰）"},
	{"POST", "/api/v1/credentials/:id/rotations/:rid/abandon"}:                       {classResource, model.ResourceCredential, "命中分類器段 `credentials`（本 change 新增常數接線）；`:id` 指向憑證列，**不是資產 id**；掛載類動作的受影響資產由 handler 顯式注入主體鍵（路由層推導不出那台機器是誰）"},
	{"PUT", "/api/v1/assets/:id/accounts/:accountId/credential"}:                     {classResource, model.ResourceAsset, "命中分類器段 `assets`（路徑序先於 `credential` 單數段，且本路由的主體確實是那台資產）"},
	...
	{"GET", "/api/v1/identity-sources"}:                                              {classResource, model.ResourceAuth, "命中分類器段 `identity-sources`（沿用 auth 分類，未新增資源常數）；映射規則的來源與規則識別刻意不叫 `:id`，故不填 resource_id"},
	{"GET", "/api/v1/identity-sources/:type/:sourceId/mappings"}:                     {classResource, model.ResourceAuth, "命中分類器段 `identity-sources`（沿用 auth 分類，未新增資源常數）；映射規則的來源與規則識別刻意不叫 `:id`，故不填 resource_id"},
	{"POST", "/api/v1/identity-sources/:type/:sourceId/mappings"}:                    {classResource, model.ResourceAuth, "命中分類器段 `identity-sources`（沿用 auth 分類，未新增資源常數）；映射規則的來源與規則識別刻意不叫 `:id`，故不填 resource_id"},
	{"PUT", "/api/v1/identity-sources/:type/:sourceId/mappings/:ruleId"}:             {classResource, model.ResourceAuth, "命中分類器段 `identity-sources`（沿用 auth 分類，未新增資源常數）；映射規則的來源與規則識別刻意不叫 `:id`，故不填 resource_id"},
	{"DELETE", "/api/v1/identity-sources/:type/:sourceId/mappings/:ruleId"}:          {classResource, model.ResourceAuth, "命中分類器段 `identity-sources`（沿用 auth 分類，未新增資源常數）；映射規則的來源與規則識別刻意不叫 `:id`，故不填 resource_id"},
	...
	{"GET", "/api/v1/ldap-directory/status"}:                                         {classResource, model.ResourceLDAPDirectory, "命中分類器段 `ldap-directory`（新增常數接線）；設定變更（非審計資料讀取），不入 auditSensitiveResources；單例，無 `:id`"},
	...
	{"POST", "/api/v1/oidc-providers/discovery-preview"}:                             {classResource, model.ResourceOIDCProvider, "命中分類器段 `oidc-providers`；探索預覽為唯讀且不落庫，另有服務層的呼叫留痕（成功與失敗各一列）"},
	{"GET", "/api/v1/oidc-providers/:id"}:                                            {classResource, model.ResourceOIDCProvider, "命中分類器段 `oidc-providers`（新增常數接線）；設定變更（非審計資料讀取），不入 auditSensitiveResources；`:id` 指向提供者列"},
	{"GET", "/api/v1/oidc-providers/:id/status"}:                                     {classResource, model.ResourceOIDCProvider, "命中分類器段 `oidc-providers`（新增常數接線）；設定變更（非審計資料讀取），不入 auditSensitiveResources；`:id` 指向提供者列"},
	...
	{"GET", "/api/v1/security-policies/defs/:key"}:                                   {classResource, model.ResourceSecurityPolicy, "命中分類器段 `security-policies`；`:key` 是設定鍵名非自增 id，resource_id 恆 nil"},
	{"POST", "/api/v1/security-policies/compliance/preview"}:                         {classResource, model.ResourceSecurityPolicy, "命中分類器段 `security-policies`；唯讀草稿判定，不寫入"},
	{"POST", "/api/v1/security-policies/apply-preview"}:                              {classResource, model.ResourceSecurityPolicy, "命中分類器段 `security-policies`；唯讀套用預覽，寫入仍走批次更新"},
	{"GET", "/api/v1/policy-groups"}:                                                 {classResource, model.ResourcePolicyGroup, "命中分類器段 `policy-groups`"},
	{"GET", "/api/v1/policy-groups/:code"}:                                           {classResource, model.ResourcePolicyGroup, "命中分類器段 `policy-groups`；`:code` 是組代號非自增 id"},
	{"POST", "/api/v1/policy-groups"}:                                                {classResource, model.ResourcePolicyGroup, "命中分類器段 `policy-groups`"},
	{"PUT", "/api/v1/policy-groups/:code"}:                                           {classResource, model.ResourcePolicyGroup, "命中分類器段 `policy-groups`"},
	{"PUT", "/api/v1/policy-groups/:code/enabled"}:                                   {classResource, model.ResourcePolicyGroup, "命中分類器段 `policy-groups`"},
	{"DELETE", "/api/v1/policy-groups/:code"}:                                        {classResource, model.ResourcePolicyGroup, "命中分類器段 `policy-groups`"},
	{"PUT", "/api/v1/policy-groups/:code/clauses/:clause_no"}:                        {classResource, model.ResourcePolicyGroup, "命中分類器段 `policy-groups`；條號在 details 而非 resource_id"},
	{"DELETE", "/api/v1/policy-groups/:code/clauses/:clause_no"}:                     {classResource, model.ResourcePolicyGroup, "命中分類器段 `policy-groups`；條號在 details 而非 resource_id"},
	...
	{"POST", "/api/v1/users/:id/roles/pin"}:                                          {classResource, model.ResourceUser, "命中分類器段 `users`"},
