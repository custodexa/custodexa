...
// 信封密文格式（key-management-envelope D2）：enc:v<version>:<base64(nonce+ct)>。
// legacy 密文為純 base64（不含 ':'），與前綴無歧義；ParseEnvelope 以此分派
// 新舊解密路徑。
//
// AAD 方案（kek-provider-modularization D5）：enc:a<scheme>:v<version>:<base64>。
// 目前唯一方案為 a1（資料層 AAD 綁 table|column；**定案 A2 後不綁 pk**，
...
const (
	...
	// （定案 A2：**pk 不參與**——理由見 codec.go 的 CipherRef）
	AADSchemeA1 = "a1"
)
...
// （**全專案唯一的資料密文編碼入口**）。
//
// **無 AAD 編碼能力已於 release-transitional-cleanup 移除**：原
