// Package gatewayapi 定義 gateway 行程邊界的對外契約（modular-architecture W1 任務 1.5）。
//
// # 定位
//
// 本包是 R3.1 §3.3 拓樸序的最底層：`pkg/gatewayapi` → {keyvault, policy} → audit →
...
//
// # 兩條 SHALL NOT（D-11，S4 codex 採納項 #7）
//
...
// # 未定案的東西不進契約
//
// `SessionLimits` 刻意不含 `RecordingRequired`（D-6 未拍板；兌換側現況零強制，
