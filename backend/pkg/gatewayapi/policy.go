...
// Stage 判定發生的閘序位置。四值不可增刪而不同步逐閘等價表。
//
// 為何要拆 RedeemTerminal／RedeemGraphical：兩個兌換入口
...
type ConnectSubject struct {
	...
	// 現查結果回填 `Decision.ResolvedRole`，審計亦只採信 ResolvedRole。
	//
	// 為何是 SHALL NOT 而不是「盡量不要」：跨行程的 caller-asserted role 是提權面，
	// 直接撞專案紅線（internal/service/auth_service.go:945-950 明文
	// 「不得憑 JWT/token 攜帶的角色快照」判定）；且即使不用於判定，只要寫進審計就是
	// 稽核脈絡偽造面。欄位保留是為了讓「呼叫端聲稱的」與
...
// SessionLimits 會話層限制。
//
// **刻意不含 RecordingRequired**（尚未拍板）：兌換側現況零強制，本 change 內
...
// （現況前端 connect.js 依 Reason／Policy／risks 陣列分支彈框，
...
type Decision struct {
	...
	//
	// **同一失敗在不同 Stage 回不同碼是刻意的防探測設計，不是不一致**：
	...
	// **審計唯一可信的角色來源**——不得改用 ConnectSubject.ClaimedRole。
	ResolvedRole string
...
//
// # 為何回傳型別是 *Denial 而非 (Decision, error)
//
...
// PolicyGate 政策閘。**兩階段契約**（訂正原先的單一 Authorize）。
//
// # 為何非拆兩階段不可
//
