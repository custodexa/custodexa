...
| AP-30 | `internal/modules/asset/asset_account_service.go:639` | RecordCall | 是 | **是** | TxSink | W6（6.1） | `internal/modules/asset/asset_account_service_test.go:327` | `Create` 的 `repository.DB.Transaction`（`:445`）閉包內，`return model.RecordAssetAccountChange(tx …)` ——error 直接成為交易回傳值 |
| AP-31 | `internal/modules/asset/asset_account_service.go:869` | RecordCall | 是 | **是** | TxSink | W6（6.1） | `internal/modules/asset/asset_account_service_test.go:327` | `Update` 的交易（`:536`）內，`return` 形態 |
| AP-32 | `internal/modules/asset/asset_account_service.go:952` | RecordCall | 是 | **是** | TxSink | W6（6.1） | `internal/modules/asset/asset_account_service_test.go:327` | `Delete` 的交易（`:647`）內，`return` 形態 |
| AP-33 | `internal/modules/asset/asset_account_service.go:1006` | RecordCall | 是 | **是** | TxSink | W6（6.1） | `internal/modules/asset/asset_account_service_test.go:327` | `SetDefault` 的交易（`:705`）內，`return` 形態 |
| AP-34 | `internal/modules/asset/asset_account_service.go:1139` | RecordCall | 是 | **是** | TxSink | W6（6.1） | 待補 | `syncDefaultAccountFromAsset(tx *gorm.DB …)`（`:816`）內，`return` 形態；唯一呼叫方 `asset_service.go:896`（在 `:932` 交易內）；行號因 credential-library 驗收修補於 `applyAccountUpdates` 補回認證方式欄與其註解而下移 3 行，程式碼未變 |
| AP-35 | `internal/modules/asset/asset_account_service.go:1213` | RecordCall | 是 | **是** | TxSink | W6（6.1） | 待補 | 同 AP-34 函式的更新分支 |
...
| AP-38 | `internal/modules/asset/asset_service.go:633` | RecordCall | 是 | **是** | TxSink | W6（6.1） | 待補 | `Create` 的交易（`:385`）內，`:414` `return fmt.Errorf("記錄預設帳號建立審計失敗: %w", err)` |
| AP-39 | `internal/modules/asset/asset_service.go:652` | RecordCall | 是 | 否 | TxSink | W6（6.1） | 待補 | 同一交易內，但 `:425` **只 `log.Printf` 不回 error** ——寫入吃 `tx`（故變體為 TxSink），呼叫方為 fail-open。收口時**保持現況 fail-open**，不得順手改成 fail-close（那是行為變更） |
| AP-40 | `internal/modules/asset/asset_service.go:914` | RecordCall | 是 | **是** | TxSink | W6（6.1） | `internal/modules/asset/change_secret_reliability_test.go` | `commitBindingSecret`（`:812`）的交易內，`return` 形態。**本點是密碼與私鑰兩條系統寫入路徑的共同產生點**：`UpdatePassword`（`:779`）與 `UpdatePrivateKey`（`:800`）自憑證庫化之後共用同一段實作，兩者的差異只剩加密用的 CipherRef、秘密型別與審計欄位名，故原本各自登記的兩列合併於此（原 AP-63 的登記見 §2.1 的已退場列）。fail-close：審計寫入失敗即整筆交易回滾，新版本不落地 |
| AP-41 | `internal/modules/asset/asset_service.go:1344` | RecordCall | 是 | 否 | TxSink | W6（6.1） | 待補 | `Update` 的交易（`:967`）內，`:999` 只記 log。同 AP-39 的 fail-open 保持 |
| AP-42 | `internal/modules/asset/asset_service.go:1352` | RecordCall | 是 | 否 | TxSink | W6（6.1） | 待補 | 同上，`:986-987` 註解自陳「不中斷事務，只記錄錯誤」 |
...
| AP-64 | `internal/modules/asset/change_secret_candidate_service.go:405` | RecordCall | 是 | 是 | TxSink | W6（6.1） | `internal/modules/asset/change_secret_reliability_test.go` | **change-secret-ssh-deepening 新增，自始為收口後形態**。`DiscardByAdmin` 的交易內：admin 顯式清除未驗證候選憑證是破壞性操作（候選為那把可能已在遠端生效的秘密的唯一副本），刪列與審計同交易。**fail-close**：審計失敗即不刪列 |
...
| AP-84 | `internal/database/credential_secret_conversion.go:575` | AuditLog | 是 | **是** | TxSink | W6（credential-library 追加；**現況為 `tx.Create` 直寫、尚未經 `port.WriteInTx`**——本點落在 `internal/database`，該包目前沒有審計落地面可用，收口須先為存量轉換備妥 sink 注入口。分派仍依 §1.3 的硬性規則標 TxSink：寫入吃的是呼叫方 `tx`，AsyncSink 的簽名表達不了它，誤標 AsyncSink 會把 fail-close 靜默換成 fail-open） | `internal/database/migration_credential_library_test.go` | **credential-library 新增**。解封後存量轉換的群組不一致留痕：既有帳號搬入憑證模型時，同一組隱性共用關係的成員解密後明文不全等即各留專用憑證，並於同一交易寫一筆 `action=migration`、`resource=credential` 的列說明該組被拆開。**直寫 `model.AuditLog` 而非經 sink**：本點落在 migration 的執行期，早於 audit 模組的落地器組裝——此處取得的是 migration 自己的 `tx`，審計失敗即整筆 migration 回滾（fail-close）。details 帶群組值與成員數，**無任何密文與明文**。**漏登記的後果**：升級時哪些共用關係被判定為不一致而拆開，是部署方事後唯一能回答「為什麼這幾台不再是同一組」的證據 |
...
| AP-86 | `internal/database/credential_secret_conversion.go:666` | AuditLog | **是（機器判 TxBound；呼叫方一律傳根 `db`，實際交易外落地）** | 否 | TxSink | W9（credential-library） | `internal/database/credential_secret_conversion_test.go`（`TestCredentialSecretConversionRecordsFailureAudit`） | **credential-library 追加**。`recordConversionFailure`：解封後憑證密文轉換每次失敗嘗試留一筆 status=failure 審計列，**刻意寫在交易之外**（`db.Create`，非 `tx`）——寫在交易內會隨回滾一起消失，等於失敗不留痕。details 只記失敗階段與待處理筆數，不含密文、明文或原始錯誤字串（原文留啟動日誌）。同步失敗只記日誌，不阻塞服務啟動。機器因 `*gorm.DB` 參數判 TxBound，唯一呼叫路徑傳的是根 `db`（交易外），登記於守衛 `auditPointTxHumanQualified` [人工複核 2026-09-07] |
...
| **AsyncSink** | **51** | AP-01…AP-21、AP-28、AP-29、AP-44…AP-49、AP-52…AP-55、AP-58、AP-59、AP-65…AP-73、AP-75、AP-76、AP-77、AP-80、AP-81、AP-82、AP-83 |
