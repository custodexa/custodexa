...
| AP-19 | `internal/api/user_handler.go:537` | AuditLogEntry | 否 | 否 | AsyncSink | W4 | 待補 | handler `Unlock`（`:426`） |
| AP-20 | `internal/api/user_handler.go:585` | AuditLogEntry | 否 | 否 | AsyncSink | W4 | 待補 | handler `SetInactivityExempt`（`:466`） |
...
| AP-44 | `internal/modules/identity/auth_service.go:860` | AuditLog | 否 | 否 | AsyncSink | W4／檔案隨 W8 搬 identity | `internal/modules/identity/auth_service_ldap_resolution_test.go:271` | `auditLDAPResolveFailure`（`:654`），`:668` `repository.DB.Create` 失敗只記 log。**2026-08-19 上移 1 行**：`password-hasher-interface` 移除本檔的 bcrypt import 致全檔位移，產生點未變，僅重新錨定。**產生點與程式碼皆未變，行號因 audit-coverage-closure 於 `LoginResponse` 增列 `AuthProviderID`／`AuthProviderName`（供 handler 標註 provider）而下移 5 行**，僅重新錨定。**2026-09-03 再上移 2 行**：`access-token-memory-only` 移除本檔已無生產呼叫端的 `VerifySession`（唯讀觀看改以一次性觀看票認證後該介面消費者歸零）致全檔位移，產生點與程式碼皆未變。**2026-09-08 再漂移**：外部群組對角色映射於同檔前段插入程式碼（登入路徑新增映射重算接點與其輔助函式），產生點與程式碼皆未變，僅重新錨定 |
| AP-45 | `internal/modules/identity/auth_service.go:883` | AuditLog | 否 | 否 | AsyncSink | W4／檔案隨 W8 搬 identity | `internal/modules/identity/auth_service_transmission_test.go:51` | `auditLDAPTransport`（`:674`），`:699` 同上。**行號下移 5 行的成因同 AP-44**，僅重新錨定。**2026-08-19 再上移 1 行**：`password-hasher-interface` 移除本檔的 `golang.org/x/crypto/bcrypt` import（改經 `pkg/crypto` 的 Hasher／Verifier），import 區塊少一行故全檔位移；產生點與程式碼皆未變。**2026-09-03 再上移 2 行**：成因同 AP-44（`VerifySession` 移除致全檔位移），僅重新錨定。本點的 `status=denied`（`ldap_strict_reject`）自 audit-coverage-closure 起計入每日覆核的登入失敗數——計數口徑以 `action=login` 收斂，故 `resource=transmission` 不再使它被排除。**2026-09-08 再漂移**：外部群組對角色映射於同檔前段插入程式碼（成因同 AP-44），產生點與程式碼皆未變，僅重新錨定 |
...
| AP-65 | `internal/api/asset_handler.go:797` | AuditEvent | 否 | 否 | AsyncSink | — | 待補 | **data-transfer-control 期 1 新增，自始為收口後形態**。`auditK8sFileDenied`（`:767`）：kubectl cp 被資料傳輸閘擋下時的 `status=denied` 留痕，與成功路徑 AP-04 對稱。**漏登記的後果**：拒絕留痕是「誰在什麼時候試圖搬走什麼」的唯一證據，未登記者在下一次 sink 收口時沒有任何東西擋著它被漏接或誤分派——而它一旦靜默失效，稽核看到的會是「沒有人嘗試過」，與「嘗試過但被擋下」在資料上無從分辨。fail-open 沿成功路徑（`_ =` 丟棄 error），刻意與 AP-04 一致 |
...
| AP-67 | `internal/proxy/handler.go:399` | AuditEvent | 否 | 否 | AsyncSink | — | 待補 | **data-transfer-control 期 1 新增**。連線建立時把「這次連線當時允許哪五種傳輸能力」快照入審計；政策可在連線後被改，只查政策現值答不出「那次連線當時允許什麼」。**產生點與程式碼皆未變，行號因 workbench-exits-and-export 在上方插入 `recordingStartedAt` 擷取而下移**（原 `:334`／守衛 `:333`；audit-coverage-closure 於上方新增兌換拒絕留痕下移至 `:362`；workbench-clipboard-and-layout 於上方新增 ClipboardEncrypt 欄位註解，再下移至 `:369`），僅重新錨定；`AssetID` 仍未填，缺口狀態不變（見 `audit_points_asset_pivot_guard_test.go` 的 AP-67 列與 `maxAssetPivotGaps`）。`:342` 以 `h.dataTransfer != nil && h.AuditSink != nil` 雙重守衛，失敗只記 log 不回壓連線（與 FileTap 同處置，刻意 fail-open）。**漏登記的後果**：這是事後回答「當時的有效能力」的唯一來源，未登記者在收口時被漏接後，能力快照消失卻不會有任何測試轉紅——傳輸爭議將無從還原當時的政策狀態  本輪位置核對：產生點 `:399`，所在函式起點 `:142`；上文舊行號保留為歷史紀錄，ID、種類、交易歸屬與分派不變。 |
...
| AP-88 | `internal/modules/identity/role_mapping_recompute.go:327` | AuditEvent | 是 | **是** | TxSink | W6（外部群組對角色映射；自始為收口後形態） | `internal/modules/identity/role_mapping_login_test.go`（套用事件與跳過事件兩條路徑） | `writeRoleMappingAudit`（`:320`）單一寫入點、**兩條呼叫路徑同為 fail-close**：套用事件自 `applyRoleMappingLocked` 收使用者級憑證鎖的 `tx`（角色列變動、世代推進與留痕同生共死，寫不進去即整筆回滾）；跳過事件自 `skipRoleMapping` 的 `db.Transaction` 閉包收 `tx`（該路徑不動任何角色列，回滾的只有這一列本身）。機器碼放錯誤訊息欄（該欄隨外送轉發，細節欄不隨），細節欄記通道、命中的群組原始值、增減的角色名與是否推進世代 [人工複核 2026-09-08]。**2026-09-08 同案再漂移**：同檔上方新增群組觀測快照的寫入函式，產生點與程式碼皆未變，僅重新錨定 |
