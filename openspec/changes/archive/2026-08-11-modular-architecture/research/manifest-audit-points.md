...
| AP-82 | `internal/sshproxy/dbconsole_export.go:354` | AuditLogEntry | 否 | 否 | AsyncSink | — | `internal/sshproxy/dbconsole_export_test.go`（`TestConsoleExportSuccessAuditFacts`／`TestConsoleExportAbortLeavesFailureAudit`） | `(*Handler).auditConsoleExport`（`:336`）：資料庫查詢結果匯出（CSV）成功／中止／被拒絕的唯一寫入點，`action=file_download`、`resource=file`。`h.AuditService.Log(entry)`（`:371`）無回傳值，失敗只記 log。`AssetID` 在字面量內（`:359`），值由 `:349-353` 依 `assetID` 是否非零算出，資產未知時為 nil；`ResourceID` 於 `:368-370` 沿用同一值 |
...
| **AsyncSink** | **50** | AP-01…AP-21、AP-28、AP-29、AP-44…AP-49、AP-52…AP-55、AP-58、AP-59、AP-65…AP-73、AP-75、AP-76、AP-77、AP-80、AP-81、AP-82 |
