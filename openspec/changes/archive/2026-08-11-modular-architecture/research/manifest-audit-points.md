...
| AP-02 | `cmd/server/stage2.go:1355` | AuditLogEntry | 否 | 否 | AsyncSink | W4 | 待補 | `logKEKSwitchAudit`（`:1005`），KEK 切換補記；行號因 workbench-clipboard-and-layout B2 於上方新增匯出服務組裝與打包 worker 而下移，程式碼未變；single-instance-guard 於 `mark("auditService")` 後注入守衛事件 sink（`:438`）再下移 4 行；evidence-offsite-storage 於 post-unseal 佇列登記處新增離機 seed 一項再下移 8 行，程式碼未變；credential-library 於改密執行器組裝處新增憑證庫服務與輪替引擎再下移 34 行，程式碼未變；credential-library 驗收修補於同處補一行註解再下移 1 行，程式碼未變。**2026-09-08 再漂移**：外部群組對角色映射於同檔前段插入程式碼（組裝根新增一行映射審計落地面的注入），產生點與程式碼皆未變，僅重新錨定 |
...
| AP-13 | `internal/api/security_policy_handler.go:186` | AuditLogEntry | 否 | 否 | AsyncSink | W4 | 待補 | `auditPolicyChange`（`:181`） |
...
| AP-43 | `internal/modules/audit/audit_log_service.go:197` | AuditLog | 否（機器不可判定） | 否 | **不進 sink** | W4 | `internal/api/key_rewrap_audit_regression_test.go:60` | `(*AuditLogService).Log`（`:123`）的 entry→row 轉換，即 AsyncSink 的實作本體（`:134` `AuditLogEnabled=false` 靜默 return、`:175` 滿載丟棄）。它是 sink 的實作側，不再經 sink。**列在 `Log` 內建構、落地在 `writeToDatabase`／`logChan` 消費端，作用域分析對交易歸屬無資訊** [人工複核 2026-08-09] |
