...
> **2026-08-22 重新錨定（行號位移，產生點與程式碼皆未變）**：
> `refresh-token-httponly-cookie` 把 refresh 憑證的瀏覽器端載體改為 httpOnly cookie，
> 於下列檔案插入程式碼致其後的審計產生點整體位移——
> `internal/api/auth_handler.go`（cookie writer 欄位／注入器與六個發放端點之一的下發呼叫）、
> `internal/api/auth_mfa_handler.go`（MFA 兩條完成路徑的 cookie 下發）、
> `internal/api/oidc_handler.go`（handler 欄位、注入器與 exchange 的 cookie 下發）、
> `internal/modules/identity/auth_service.go`（`LoginResponse` 的 `json:"-"` 說明與
> `RefreshExpiresAt` 欄位）。
> 受影響：AP-06／07／08／09／10／44／45／72／73，**僅重新錨定行號，語義與變體歸屬皆未動**
> ——本 change 未新增、未移除、未搬動任何審計產生點，審計射程零變動。
>
...
| AP-06 | `internal/api/auth_handler.go:326` | AuditLogEntry | 否 | 否 | AsyncSink | W4 | `internal/api/auth_audit_source_and_provider_test.go` | `auditPasswordNoncompliant`（`:160`）。**行號因 audit-coverage-closure 批 7 於檔頭新增 `trustProxy` 欄位與 `auditSourceIP` 而下移**，產生點未變；`ClientIP` 改由 `h.auditSourceIP(c)` 取（不採信轉送標頭） |
| AP-07 | `internal/api/auth_handler.go:345` | AuditLogEntry | 否 | 否 | AsyncSink | W4 | `internal/api/auth_audit_source_and_provider_test.go` | `auditLogin`（`:181`）。同 AP-06：行號因批 7 下移，`ClientIP` 改取連線對端 |
...
| AP-09 | `internal/api/auth_handler.go:676` | AuditLogEntry | 否 | 否 | AsyncSink | W4 | 待補 | `auditPasswordChange`（`:488`）。**行號因 audit-coverage-closure 批 4 於上方新增 refresh 成功留痕、批 7 於檔頭新增 `auditSourceIP` 而兩度下移**，產生點與程式碼皆未變 |
| AP-10 | `internal/api/auth_mfa_handler.go:318` | AuditLogEntry | 否 | 否 | AsyncSink | W4 | `internal/api/auth_audit_source_and_provider_test.go` | 產生點自 `auditAuthEventWithResource` 下沉為 `auditAuthEventFull`（`:310`，落地在 `:328` `Log(entry)`），前者變成薄包覆——**本檔審計列仍只有這一個字面量**，audit-coverage-closure 2.9 只是替它加上 `Details` 參數（MFA 完成路徑的 provider 標註）。同批 7：`ClientIP` 改取連線對端 |
...
| AP-45 | `internal/modules/identity/auth_service.go:760` | AuditLog | 否 | 否 | AsyncSink | W4／檔案隨 W8 搬 identity | `internal/modules/identity/auth_service_transmission_test.go:51` | `auditLDAPTransport`（`:674`），`:699` 同上。**行號下移 5 行的成因同 AP-44**，僅重新錨定。**2026-08-19 再上移 1 行**：`password-hasher-interface` 移除本檔的 `golang.org/x/crypto/bcrypt` import（改經 `pkg/crypto` 的 Hasher／Verifier），import 區塊少一行故全檔位移；產生點與程式碼皆未變。本點的 `status=denied`（`ldap_strict_reject`）自 audit-coverage-closure 批 3 起計入每日覆核的登入失敗數——計數口徑以 `action=login` 收斂，故 `resource=transmission` 不再使它被排除 |
