...
| H-13 | hook:cmd/server/stage2.go:keyvault.RegisterPostUnsealBuiltin | `keyvault.RegisterPostUnsealBuiltin(service.PostUnsealMigrationLDAPSeed, service.RegisterLDAPSeedMigration)` | cmd/server/stage2.go:343 | 套件層註冊 | assembly（已自 keyvault 上移） | W1（1.10，已完成） | **必須早於下一行的 `RunPostUnsealMigrations`（`:259`），且兩者都必須晚於 H-10**。登記動作已上移組裝根以拆 4.9 keyvault↔identity 環：identity 匯出自己的 `RegisterLDAPSeedMigration`，由本呼叫注入 keyvault 的登記器清單（G-60b）。**日後每加一個內建遷移即多一行登記，全部 SHALL 落在 `Run` 之前**，否則佇列在執行時只有一半——`post_unseal_guard_test.go` 的 `TestAssemblyRegistersPostUnsealBuiltins` 對本檔逐條斷言（含具名清單，防止某一行被刪後包內測試仍綠）。 |
...
| H-112 | hook:cmd/server/stage2.go:keyManager.SetDEKFailureReporter | `keyManager.SetDEKFailureReporter(auditFailureService)` | cmd/server/stage2.go:617 | setter 後綁定（窄介面注入） | keyvault ← audit | 資料金鑰隨用隨解 | 資料金鑰解封連續失敗的失效事件上報面。**必須晚於 `InitAuditFailure` 與告警服務就緒**（同 `kekRetirementMonitor` 的既有理由：更早注入時首批通知必被丟棄）。漏注入不報錯——連續解封失敗只剩行程日誌，`audit_failure_events` 無列、通知通道無訊息，而那正是「保管處連不上、資料現在讀不出來」唯一會傳到人身上的路徑。 |
| H-113 | hook:cmd/server/stage2.go:s1.metrics.RegisterDEKUnwrap | `s1.metrics.RegisterDEKUnwrap()` | cmd/server/stage2.go:1171 | 分階段註冊（指標曝光面） | assembly ← observability | 資料金鑰隨用隨解 | 註冊資料金鑰解封的四條序列。理由同 H-59：封印期沒有可解封的金鑰，序列缺席才正確表達「服務不存在」。以 `sync.Once` 保證冪等——B 模式每次解封都會走到，重複 `MustRegister` 會 panic 並終止行程。 |
| H-114 | hook:cmd/server/stage2.go:keyManager.SetDEKObserver | `keyManager.SetDEKObserver(s1.metrics)` | cmd/server/stage2.go:1172 | setter 後綁定（觀測掛勾） | keyvault ← observability | 資料金鑰隨用隨解 | 解封結果、耗時、在途數與現行設定值的出口。**須緊接 H-113**：註冊之前寫入是無害的（曝光與否由註冊決定），但反過來註冊了卻沒注入，四條序列會恆為零值——採集端讀到的是「解封從未失敗、從未發生」，與「機制沒接上」無從分辨。 |
...
| H-86 | hook:cmd/server/stage2.go:s1.metrics.RegisterOffsiteInventory | `s1.metrics.RegisterOffsiteInventory()` | cmd/server/stage2.go:1034 | 單例註冊（`sync.Once`） | assembly → observability | —（evidence-offsite-storage 1.10） | 離機存量與失敗面的序列註冊；**條件＝設定世代總列數 > 0**（含停用態）。須早於 `StartRefresher`——反了則首輪刷新寫進尚未註冊的 collector，該輪資料在 `/metrics` 上看不到（值本身不會遺失，下一輪覆蓋，但首輪缺口在 30 秒內存在）。`sync.Once` 使 B 模式重複解封不重複註冊（重複註冊會 panic）。 |
| H-87 | hook:cmd/server/stage2.go:s1.metrics.RegisterOffsiteUploadLane | `s1.metrics.RegisterOffsiteUploadLane()` | cmd/server/stage2.go:1037 | 單例註冊（`sync.Once`） | assembly → observability | —（evidence-offsite-storage 1.10） | 離機上傳車道的序列註冊；**條件＝有現行世代**。停用態下不註冊是刻意的（停用態表）：worker 不存在時若序列還在，採集端讀到「待上傳恆為 0、最後成功時刻停在過去」，與「一切正常且無事可做」無從分辨，而 `absent()` 能明確表達前者。與 H-86 的條件不同，**兩者不得合併**。 |
| H-88 | hook:cmd/server/stage2.go:offsiteProfiles.SetUploaderStarter | `offsiteProfiles.SetUploaderStarter(startOffsiteUploader)` | cmd/server/stage2.go:995 | setter 後綁定（延後啟動的回呼） | assembly ← offsite | evidence-offsite-storage | 零現行世代啟動時上傳 worker 不建 goroutine（ST-34c2），本回呼是它其後**唯一**的啟動路徑：管理員於管理介面完成首次設定時由設定服務呼叫，不需重啟。**必須早於路由掛載**（本行在段 2 建構期，故成立）——晚於路由即存在「設定已可寫入而回呼還沒接上」的窗口，落在該窗口的那次儲存等於沒設定。漏注入不產生任何錯誤：設定寫得進去、頁面顯示已啟用、帳冊照排，只是沒有任何人領件，全部停在待上傳直到下次重啟。回呼自身冪等（`sync.Once`），故就地更新與世代切換都照呼叫，不必在呼叫端判斷「這次是不是從零到有」。 |
...
| H-92 | hook:cmd/server/stage2.go:instanceGuardHaltHandler.SetSourceControls | `instanceGuardHaltHandler.SetSourceControls(...)`（完整路由樹） | cmd/server/stage2.go:1555 | setter 後綁定 | assembly（不搬） | preservice-pages | 同 H-91 的正常服務期路徑：兩條攔下端點在完整路由樹上恆註冊（使路由 golden、審計分類與 API 索引看得見它們），故正常服務期同樣需要來源限制。漏接不會讓功能壞掉——該狀態下兩端點在觸碰任何憑證之前即回 running／409——但那道網段限制就沒生效，屬靜默放寬。 |
| H-101 | hook:cmd/server/stage2.go:checkpointService.SetRoleStateReconciler | `checkpointService.SetRoleStateReconciler(roleStateReconciler)` | cmd/server/stage2.go:813 | setter 後綁定 | audit | —（role-assignment-integrity 新增） | 封章前的角色指派對帳。**三個時機（本列、H-102、H-103）SHALL 共用同一個對帳器實例**：冪等鍵存在實例上，各建一份會讓同一筆不符在三個時機各開一張單。**必須早於首次封章**（本行在段 2 建構期，排程器於其後啟動，故成立）；漏注入時檢查點的對帳結果欄恆為未附帶，鏈照封、驗證頁那一列說「本次未取得比對結果」——**不是相符**，這個區別是本機制的全部價值所在。對帳失敗不阻止封章（與快照失敗不同）。 |
| H-102 | hook:cmd/server/stage2.go:checkpointVerifier.SetRoleStateReconciler | `checkpointVerifier.SetRoleStateReconciler(roleStateReconciler)` | cmd/server/stage2.go:814 | setter 後綁定 | audit | —（role-assignment-integrity 新增） | 驗證端點與排程自動驗證的角色指派維度（兩者同走 `VerifyChain`，故一處接線涵蓋兩條路徑）。同 H-101 的共用實例約束；**必須早於路由掛載**（本行在段 2 建構期）。漏注入時 `ChainReport.RoleState` 為 nil，驗證頁顯示「取不到比對結果」而非留白——看不到區塊的稽核會讀成「沒有這個機制」。 |
| H-103 | hook:cmd/server/stage2.go:authService.SetRoleStateProbe | `authService.SetRoleStateProbe(roleStateReconciler)` | cmd/server/stage2.go:815 | setter 後綁定 | assembly ← identity／audit | —（role-assignment-integrity 新增） | 特權登入（admin／auditor）取得使用者之後、**任何權杖簽發之前**的比對探針。同 H-101 的共用實例約束。介面 `RoleStateProbe` 無回傳值，故「比對不阻斷登入」由型別保證，接線順序不會改變登入的成敗。**必須早於路由掛載**（登入端點在其後掛上）；漏注入時登入照常、只是直寫提權要等到下一次封章或驗證才被看見——偵測延遲從「下一次特權登入」拉長到「下一輪排程」。 |
| H-104 | hook:cmd/server/stage2.go:auditCheckpointHandler.SetRoleStateNames | `auditCheckpointHandler.SetRoleStateNames(s.userService, s.auditFailureService)` | cmd/server/stage2.go:1508 | setter 後綁定 | api ← identity／audit | —（role-assignment-integrity 新增） | 角色指派差集的名稱換算與失效事件連結。**兩個來源都是模組服務、不是 `*gorm.DB`**：接入層自持連線直查即多一份「帳號叫什麼名字」的真相（`TestAPILayerHasNoDirectModelQuery`）。**必須早於本 handler 的路由掛載**（同一函式內、本行在其上方）。漏注入不隱藏區塊：差集仍出識別、狀態仍出，只是沒有名字也沒有事件連結——稽核讀得到「差在 (7, 1)」卻看不到「zhangsan 多了 admin」。 |
| H-105 | hook:cmd/server/stage2.go:audit.SetUserRolesSource | `audit.SetUserRolesSource(identity.SnapshotUserRolePairs)` | cmd/server/stage2.go:796 | setter 後綁定 | assembly ← identity／audit | —（role-assignment-integrity 發版前補：資料邊界守衛） | 狀態表快照的 `user_roles` 讀取來源。表歸 identity，audit 只拿 (user_id, role_id) 集合；排序、去重、編碼與雜湊留在 audit。**必須早於任何封章與對帳**（同段的 checkpointService 與 roleStateReconciler 皆在其後建立）；未注入時 `SnapshotUserRoles` 回 `ErrUserRolesSourceUnset`（fail-close），不會拿空集合封進鏈裡。 |
| H-106 | hook:cmd/server/stage2.go:authService.SetRoleMappingAuditSink | `authService.SetRoleMappingAuditSink(auditTxSink)` | cmd/server/stage2.go:414 | setter 後綁定 | identity ← audit | —（外部群組對角色映射新增） | 登入重算改變角色時的交易內審計落地面。**必須早於路由掛載**（登入端點在其後掛上）。漏注入時**不是靜默不留痕**：未接線的落地面回哨兵錯誤，重算連同該次登入一併失敗——角色因外部群組而變動卻沒有任何紀錄，是這條路徑最不能出的錯，故取 fail-close。未設群組屬性名的部署完全短路、走不到這裡，故漏注入不影響既有部署。 |
| H-107 | hook:cmd/server/stage2.go:oidcLoginService.SetRoleMappingAuditSink | `oidcLoginService.SetRoleMappingAuditSink(auditTxSink)` | cmd/server/stage2.go:622 | setter 後綁定 | identity ← audit | —（外部群組對角色映射新增） | 提供者途徑登入重算的交易內審計落地面。理由同 H-106：**必須早於路由掛載**；漏注入時未接線的落地面回哨兵錯誤，重算連同該次登入一併失敗（fail-close），不是靜默不留痕。未設群組宣告名的 provider 完全短路、走不到這裡，故漏注入不影響既有部署。 |
| H-108 | hook:cmd/server/stage2.go:identitySourceService.SetDiscovery | `identitySourceService.SetDiscovery(oidcDiscovery)` | cmd/server/stage2.go:627 | setter 後綁定 | identity | —（外部群組對角色映射的管理面新增） | 狀態彙總的「探索文件可達」燈號來源。**只讀 discovery 的快取、不撥號**，故漏注入不影響任何登入路徑，症狀是該燈號恆為「尚無資料」（灰）——不會被誤讀成「探索失敗」。 |
| H-109 | hook:cmd/server/stage2.go:ldapDirectoryHandler.SetIdentitySources | `ldapDirectoryHandler.SetIdentitySources(s.identitySourceService)` | cmd/server/stage2.go:1558 | setter 後綁定 | assembly ← identity | —（外部群組對角色映射的管理面新增） | 目錄狀態彙總端點的服務來源；**必須早於路由掛載**。漏注入時該端點回可辨識的 500（未接線哨兵），不是靜默回一份全綠的假狀態。 |
| H-110 | hook:cmd/server/stage2.go:oidcHandler.SetIdentitySources | `oidcHandler.SetIdentitySources(s.identitySourceService)` | cmd/server/stage2.go:1559 | setter 後綁定 | assembly ← identity | —（外部群組對角色映射的管理面新增） | 提供者狀態彙總與探索預覽的服務來源；**必須早於路由掛載**。漏注入時兩支端點各回可辨識的失敗，不是靜默成功。 |
...
| H-118 | hook:cmd/server/stage2.go:keyManagementHandler.SetDeploymentKMSProvider | `keyManagementHandler.SetDeploymentKMSProvider(...)` | cmd/server/stage2.go:1658 | setter 後綁定 | assembly ← config | 委託拓撲與憑證改由介面管理 | 拓撲端點的服務商來源。**服務商不可經介面改**：金鑰列的 KEK 引用已釘死哪一家能解，故它只能來自部署檔。漏注入時服務商為空字串 ⇒ 無可編輯欄位 ⇒ `PUT` 一律回「該服務商沒有可編輯的拓撲欄位」，失敗方向是拒絕而非放行。 |
| H-70 | hook:cmd/server/stage2.go:authHandler.SetSourceIPBaseline | `authHandler.SetSourceIPBaseline(s.sourceIPBaseline)` | cmd/server/stage2.go:1159 | setter 後綁定 | api ← audit | —（來源限定功能） | 帳號 × 來源位址「已見」基準的注入（本地認證流）。消費端 `internal/api/auth_source_ip_observe.go` 為 `baseline == nil` 即 return——**漏注入＝五個正式會話發放點全部不觀察，且完全無錯誤訊號**：基準表永遠是空的，於是每一次建線都判為新位址、告警不停響；或反過來說，「這個帳號從沒從這裡登入過」這件事永遠答不出來。**必須與 J-14／J-15 取到同一份服務**（同一個 `sourceIPBaseline` 變數）——登入點與建線點寫的是同一張表的同一組鍵，服務分裂即判定分裂。 |
| H-71 | hook:cmd/server/stage2.go:oidcHandler.SetSourceIPBaseline | `oidcHandler.SetSourceIPBaseline(s.sourceIPBaseline)` | cmd/server/stage2.go:1160 | setter 後綁定 | api ← audit | —（來源限定功能） | 同 H-70 的 OIDC 交換流。**兩者缺任一側，另一側都不會轉紅**——症狀是「本地登入進得了基準、OIDC 登入進不了」，而該帳號自 OIDC 進來的位址從此永遠不算已見。 |
| H-72 | hook:cmd/server/stage2.go:authHandler.SetSourcePolicyReader | `authHandler.SetSourcePolicyReader(s.authService)` | cmd/server/stage2.go:1165 | setter 後綁定 | api ← identity | —（來源限定功能） | 允許來源網段的現讀面注入（本地認證流）。**與 H-70 的方向相反**：那是旁路功能，漏注入即靜默不觀察；本列是**強制點**，漏注入即 fail-close——判定點讀不到清單一律拒絕，症狀是所有人登不進去。方向刻意如此：一條漏接的組裝路徑不得讓整套來源限定靜默關掉。三個 handler（H-72／H-73／H-74）必須取到同一份服務，否則會出現「登入判、管理端點不判」這種只有一半生效的狀態。 |
...
| J-14 | inject:cmd/server/stage2.go:sshHandler.SourceIPBaseline | `sshHandler.SourceIPBaseline = sourceIPBaseline` | cmd/server/stage2.go:592 | 裸欄位注入（連線層 ← audit） | 接入層 ← audit | —（來源限定功能） | 文字終端建線點的新來源位址觀察。消費端 `internal/sshproxy/source_ip_observe.go` 為 `== nil` 即 return——**漏注入＝自新位址建線零告警、零基準列，且沒有任何錯誤或降級訊號**，事後查告警頁只看到「這段期間沒有新位址」，與真的沒有新位址不可分辨。**必須晚於 `alertSink` 與 `auditTxSink` 建構**（基準服務以兩者為建構參數）。 |
| J-15 | inject:cmd/server/stage2.go:connHandler.SourceIPBaseline | `connHandler.SourceIPBaseline = sourceIPBaseline` | cmd/server/stage2.go:593 | 裸欄位注入（連線層 ← audit） | 接入層 ← audit | —（來源限定功能） | 同 J-14 的圖形協議側。**兩側共用同一份服務是契約**：分開建兩份不會編譯失敗，但「同帳號同位址只響一次」會退化成「每種協議各響一次」。缺任一側另一側都不轉紅，症狀是「SSH 會響、RDP 不響」。 |
...
| ST-15b | step:offsiteProfiles | `offsiteCustodyJournal{tx,db,async}` ＋ `NewOffsiteProfileService` ＋ `NewLedger` ＋ `SetLedger` | cmd/server/stage2.go:461 | 啟動步驟 | offsite ← keyvault／audit | —（evidence-offsite-storage 1.10） | **必須晚於 ST-15**：保管鏈事件的非同步落地面就是 `auditService`，早於它建構只能拿到 nil，結果是 worker 每次上傳同步寫一列審計（旁路功能的成本壓在熱路徑上）。亦必須晚於 ST-1（憑證走信封加密）。**本步與是否啟用無關**——被動物件、零 goroutine、零指標註冊；「設定表零列＝行為完全不變」由 ST-34c2 不啟動 worker、指標不註冊、排隊點自我早退三者承擔。 |
| ST-15c | step:offsiteLedger | `offsiteLedger.CheckProfileContinuity()` 健檢 | cmd/server/stage2.go:475 | 啟動步驟（fail-close 健檢） | offsite | —（evidence-offsite-storage 1.10） | **沒有任何「世代拒啟」**：本健檢只斷言帳冊出現的 `storage_generation_id` ⊆ 世代表（**含已退役者——退役是合法歸屬**）。違反＝資料損壞（部分還原、DB 手術），此時繼續啟動只會讓取回以「用現行設定猜」收場，故 `return fail(...)`。訊息指名對不上的世代與物件數，**不含端點**。 |
...
| R-14 | release:cmd/server/stage2.go:vaultClient | `vaultOwner.Close()` | cmd/server/stage2.go:200 | cancel renewal and wait | assembly ← vaulttransit | Vault lifecycle | 登記於 localKEK 之後、keyManager 之前；LIFO 先排空借用服務與 keyManager，再關閉共用 client，原始 local KEK 最後歸零。部分建構失敗同樣由 ResourceBag 收束。 |
| R-2 | release:cmd/server/stage2.go:keyManager | `keyManager.ZeroizeForRelease()` | cmd/server/stage2.go:184 | zeroize | keyvault | W2 | **第 3 個登記 ⇒ 倒數第 3 個執行**（其後僅共用 Vault client 與原始 local KEK）。全部 KEK 衍生材料的持有者都排在它之後登記（故先釋放），此序不可反轉：先歸零根材料會讓仍在收束的推送器無法解密其持有的通道明文（雖然它是覆寫記憶體而非需要解密，但 `ciphers` 快取被清空後任何殘留解密路徑即失敗）。 |
