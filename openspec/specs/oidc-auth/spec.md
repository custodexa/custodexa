...
使用者憑證世代 SHALL 於「該使用者既有憑證應失效」時推進——管理者的顯式動作（帳號停用、刪除、改為僅外部登入、解除外部身分綁定、**變更其角色集**）與使用者自身的密碼變更皆屬之；**登入時依外部群組映射重算而使有效角色集有列被移除者亦屬之**——本清單 SHALL NOT 被讀成僅涵蓋管理者發動的路徑——其中角色變更 SHALL 於「有效角色集有列被移除」時推進，未移除任何列時 SHALL NOT 推進——判準 SHALL NOT 為「新舊集合是否相同」，亦 SHALL NOT 為「角色數量是否變少」；替換端點與純追加的冪等端點 SHALL 適用同一判準；**任何換發路徑所簽出的憑證 SHALL 攜帶簽發當下現查的世代，SHALL NOT 繼承來源憑證的世代值**（否則推進世代的操作本身會使其換發的新憑證立即失效，例如強制改密者將永久無法取得可用會話）；**因連續認證失敗而自動鎖定 SHALL NOT 推進**——鎖定可由未認證第三方觸發，若使既有存取立即失效即成為遠端斷線武器（既有設計刻意只撤刷新憑證、保留既有存取憑證與協議會話），使**尚未兌換**的能力憑證一併失效——僅掃描既有連線無法涵蓋這些憑證，且身分重新綁定時舊憑證會復活。
...
#### Scenario: 交棒憑證帶推進後的世代

- **WHEN** 某使用者於本次登入依外部群組映射被縮減角色而觸發世代推進，其後於同一次流程簽出交棒憑證
- **THEN** 該憑證攜帶推進**後**的世代，兌換時的世代比對通過，該使用者取得可用會話
...
系統 SHALL NOT 為取得群組而額外呼叫使用者資訊端點或保存提供者的刷新權杖：既有設計不索取離線存取範圍，會話生命週期不依賴提供者的權杖。群組資訊 SHALL 只取自登入當次的身分權杖。

群組宣告的值 SHALL NOT 被寫入使用者本體的任何識別欄位；其快照 SHALL 依外部自報值的既有紀律保存與呈現。

#### Scenario: 未設定即關閉

- **WHEN** provider 未設定群組宣告名稱而使用者經其登入
- **THEN** 登入照既有流程完成，未產生任何角色列變更；該 provider 已有啟用中的映射規則時另寫入一筆跳過事件

#### Scenario: 鍵存在則據以重算

- **WHEN** provider 已設定群組宣告名稱且身分權杖含該鍵的字串陣列
- **THEN** 依該陣列完整重算映射角色

#### Scenario: 型別不符視為未知

- **WHEN** 已設定的群組宣告在身分權杖中為非字串陣列的型別
- **THEN** 既有映射列全數保留、憑證世代不推進，並寫入可辨識的跳過事件

#### Scenario: 不為群組另呼叫端點

- **WHEN** 身分權杖不含群組宣告而 provider 已設定該名稱
- **THEN** 系統視為空集合，SHALL NOT 呼叫使用者資訊端點補取
...
管理介面 SHALL 顯示各項的系統預設值，並 SHALL 明示留空即採用該預設。

#### Scenario: 未設定即維持現行解析

- **WHEN** provider 的三項宣告對應皆未設定而使用者經其登入
- **THEN** 使用者名稱、電郵與顯示名稱的取值結果與本能力變更前相同

#### Scenario: 設定後改讀指定宣告

- **WHEN** provider 指定使用者名稱取自某個宣告，且身分權杖含該宣告
- **THEN** 該次登入以該宣告的值作為使用者名稱來源

#### Scenario: 宣告對應變更不改寫身分關聯

- **WHEN** 管理者變更某 provider 的宣告對應後，既有使用者再次經其登入
...
該預覽 SHALL 沿用既有的身分提供者出站信任邊界——issuer 形式驗證、內部位址的出站限制、release 模式對明文 issuer 的拒絕、專用的出站用戶端與其逾時——SHALL NOT 另建撥號路徑。否則此端點會成為一支可指向任意內部位址的探測器，其回應足以區分「連得上」與「連不上」。

預覽結果 SHALL NOT 落庫、SHALL NOT 改變任何 provider 的現行設定：它是一次查詢，不是儲存動作。

預覽 SHALL 限於具管理權限者，且其呼叫 SHALL 留痕。

#### Scenario: 預覽回端點與支援的宣告

- **WHEN** 管理者對一個可達的 issuer 執行探索預覽
- **THEN** 回應含其授權、權杖、簽章金鑰等端點與支援的宣告清單

#### Scenario: 內部位址的預覽被拒

- **WHEN** 管理者以指向內部位址的 issuer 執行探索預覽，而該位址不在允許清單內
- **THEN** 請求被拒並回可辨識的機器碼，SHALL NOT 發出任何對該位址的請求

#### Scenario: 預覽不改變設定

- **WHEN** 管理者對某個已存在的 provider 執行探索預覽後未儲存
- **THEN** 該 provider 的設定與其登入行為皆未變動
...
- **WHEN** 管理者開啟某 provider 的詳情且系統已設定對外基準網址
- **THEN** 回應含由該基準網址算出的回呼網址，介面可一鍵複製
