...
**信封盤點的涵蓋完整性**：引用掃描的「全部信封加密欄位」SHALL 以信封加密盤點清單（envelopeMigrationTargets）為列舉來源，
該清單 SHALL 涵蓋每一張持有信封密文的表——含憑證的密文版本表（現行與待生效版本同表）與輪替候選秘密表。
新增任何信封密文表 SHALL 於同一份變更內入冊；密文欄位自既有表搬遷至新表時，盤點清單 SHALL 同版更新，
SHALL NOT 因欄位搬家而在清單中留下缺口——清單漏一張表的失敗方向是「靜默低報引用數」，
使仍被引用的金鑰材料被判為零引用而可銷毀，該側沒有任何其他訊號。
...
#### Scenario: 憑證密文表納入引用掃描
- **WHEN** 清理範圍含一個仍被憑證密文版本或輪替候選秘密引用的退役 DEK 版本
- **THEN** 引用掃描 MUST 計入這兩張表的密文並拒清該版本，MUST NOT 因其為新引入的表而回報零引用；DEK 輪替的重加密亦 MUST 涵蓋這兩張表
