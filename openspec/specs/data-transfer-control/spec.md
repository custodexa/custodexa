...
**交付範圍（期別）**：本規格只描述已交付的全域政策鍵層。per-authorization 傳輸放寬（`authorization_transfer_grants` 子表、`全域值 OR 匹配授權放寬` 的解析、接入通道維度的寫入面、授權放寬偏離計數與鑽取）**尚未實作**，其條文刻意不收於本規格；該期別的設計與待辦保留於維護者的私有開發歷程（未隨公開倉庫發佈）。因此本規格中「有效能力」的解析結果目前恆等於全域鍵值。
## Requirements
