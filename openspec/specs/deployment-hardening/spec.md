...
### Requirement: guacd 容器以最小權限執行

專案交付的編排定義 SHALL 使圖形協議代理（guacd）容器以最小權限執行：丟棄全部 Linux capability、
拒絕經 setuid 檔案提權、根檔案系統唯讀。理由是該容器解析的是被納管主機送回的協議資料，
而該端在本產品的威脅模型中即假設可能已被入侵。

唯讀之下 SHALL 只保留該工作負載**實測**需要的可寫路徑，且 SHALL 以記憶體檔案系統供給：
RDP 磁碟重導每連線暫存目錄所在的位置，以及 RDP 函式庫啟動時要求可寫的執行身分家目錄。
可寫路徑的清單 SHALL 由實跑決定而非預防性給予。**可用性問題 SHALL NOT 以放寬上述三項換取**——
缺可寫路徑造成的失敗，正確處置是補上該路徑，不是取消唯讀根或還原能力集。
錄影目錄維持既有的共享掛載點，不受唯讀根影響。

這三項 SHALL 是**容器的執行期屬性**，而非映像內容物的性質：部署方將映像替換為自建或自選者之後
SHALL 仍然生效。文件 SHALL 載明替換映像的完整契約——在既定埠提供該協議、能寫入編排掛入的錄影目錄、
內含 `sh` 與 `nc` 供編排施加的健康檢查執行——且 SHALL NOT 於該契約之外增加對映像內容物的要求。
第三項是健康檢查沿用映像自帶判定指令、又以 `service_healthy` 為啟動前提的必然結果：缺它的映像
會停在 `starting` 而使 backend 永不啟動，故它屬於契約而非額外要求。

開發版編排 SHALL 施加與正式版相同的三項。兩邊不一致會使唯讀根與能力丟棄造成的失敗只在正式部署上出現。

編排 SHALL 覆寫該映像自帶的健康檢查週期，使節奏適合啟動序；判定指令 SHALL 沿用映像自帶者，
SHALL NOT 另行發明判定方式。backend SHALL 等待該容器通過健康檢查後才啟動，SHALL NOT 只等容器建立——
只等建立會使冷啟動的首條圖形連線落在服務尚未就緒的窗口內。

#### Scenario: 硬化後仍可建立圖形會話並落地錄影

- **WHEN** 以硬化後的編排啟動 guacd，對 RDP 與 VNC 目標各建立一段會話並持有一分鐘
- **THEN** 兩者皆連線成功並持續串流畫面更新，錄影寫入共享掛載的錄影目錄且檔案非空；
  RDP 的磁碟重導通道建立成功，其每連線暫存目錄建於保留的可寫區內

#### Scenario: 缺少可寫家目錄時 RDP 於協商階段失敗

- **WHEN** 根檔案系統唯讀而未提供執行身分可寫的家目錄
- **THEN** RDP 連線在安全協商階段即失敗（VNC 不受影響）；正確處置為補上該可寫區，
  SHALL NOT 為此取消唯讀根或還原能力集

#### Scenario: backend 等待 guacd 通過健康檢查

- **WHEN** 冷啟動整套服務
- **THEN** backend 於 guacd 通過健康檢查之後才啟動；等待時間由編排覆寫的檢查週期決定，
  SHALL NOT 取決於映像自帶的預設週期

#### Scenario: 替換映像後最小權限仍生效

- **WHEN** 部署方把該映像換成自建或自選者，且新映像在既定埠提供該協議、可寫入掛入的錄影目錄、
  內含 `sh` 與 `nc`
