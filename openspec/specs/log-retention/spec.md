...
**錄影的遠端物件**（自 `evidence-offsite-storage` 起，帳冊有列時）：錄影保留到期的清除 SHALL 只及於本機檔案與資料庫側追蹤——本機檔（若仍在）移除、保管帳冊列依逐狀態到期轉移標記（見主 spec「本機副本保留與到期清理」）、以系統主體寫入到期事件；**系統 SHALL NOT 對遠端物件發出任何刪除呼叫**。清除 SHALL 涵蓋本機檔已被本機副本保留鍵提前清除、僅剩遠端追蹤的會話（該類會話不再出現於本機目錄遍歷，SHALL 以會話紀錄驅動）。遠端物件的到期清理由部署方的 bucket 生命週期規則承擔；營運文件 SHALL 明載產品保留政策與 bucket 生命週期為兩個各自運行的期限。本機清除與水位前進 SHALL NOT 受遠端狀態影響。
...
#### Scenario: 已離機錄影到期只清本機
- **WHEN** 錄影保留政策到期，某會話的錄影已離機保存
- **THEN** 本機檔（若仍在）被移除、帳冊列標記為本機已清並寫入到期事件；系統對遠端不發出任何刪除呼叫，遠端物件的去留由部署方的 bucket 生命週期決定

#### Scenario: 舊儲存設定世代的物件到期處置
- **WHEN** 錄影保留政策到期，該會話的遠端物件屬於已切換前的儲存設定世代
- **THEN** 本機檔依政策清除、帳冊列維持舊世代語義並留痕；遠端物件一如其他世代不被產品刪除
...
已離機保存錄影的**本機副本提前清除**（本機副本保留鍵）SHALL NOT 推進錄影類水位，亦 SHALL NOT 使該會話呈現為已清除——錄影仍可自物件儲存回放；只有錄影保留政策到期的清除才前進水位。
...
#### Scenario: 本機快取清除不推進水位

- **WHEN** 本機副本保留鍵使一批已離機錄影的本機檔被清除
- **THEN** 錄影類水位不變，該批會話於工作台與會話詳情仍呈現為可回放
