...
4. **不得裸露實作術語**：SHALL NOT 包含內部函式名、狀態機器碼，或未經替換的工程行話（信封密文、解包／unwrap、收束、留痕、
   ...
   SHALL NOT 視為行話，SHALL 原樣保留。委託模式必要的識別字（保管處位址、角色識別、金鑰資源名稱）同屬技術識別字，
   SHALL 原樣呈現以供核對，SHALL NOT 為求淺白而截斷或改寫。
5. **三語同標準**：en-US 與 ja-JP SHALL 各自符合上列四條，SHALL NOT 因翻譯而回復長句或補回被刪除的設計理由。
   否定句與警告之強度 SHALL 三語一致——弱化即為缺陷。

**用詞統一**：三語 SHALL 各自對「封存（動作）／已封存（狀態）／解封（恢復）」維持一組固定譯詞，
「封印」「還原」SHALL 自可見面退場；審計檢查點鏈的同名概念 SHALL 改稱「存證」及其衍生詞。
同一概念在任兩處出現不同說法 SHALL 視為缺陷。

上述規範 SHALL NOT 被用來刪除或弱化任何事實：改寫前後**操作者做決定所需的事實條數 SHALL NOT 減少**。
原文所列的必留事實中，「一般解封不需要帳號密碼」SHALL 刪除——自解封頁前置管理者驗證起，該陳述為假，
留著它比沒有它更糟。取而代之 SHALL 新增下列必留事實：
「解封前要先用平常登入的帳號密碼確認身分」「服務暫停期間不核對動態驗證碼」
「憑證只留在後端記憶體，封存或重啟後要重新提供」「送出憑證前先核對畫面上的保管處位址與金鑰，不相符就停下來」
「連續失敗會暫停嘗試一段時間，倒數結束自動恢復、不需重啟」。
其餘既有必留事實（「主金鑰的三種輸入寫法」「另一實例仍在執行時停止它即可、不需確認」
「確認後兩實例並存的後果由確認者承擔」一類）SHALL 維持；任何「會發生 X」SHALL NOT 被改寫為「不會發生 X」。
收進展開區的事實 SHALL 於展開後完整可讀。
...
委託模式的解封頁 SHALL 以對應的陳述取代之：主金鑰由外部保管處持有，保管處或其憑證失去即全部資料不可解，系統同樣不提供救援。
...
（鍵改名或 namespace 調整導致掃空時 SHALL NOT 靜默通過）。行話黑名單 SHALL 併入退場用詞（「封印」「還原」），
其釘定的葉鍵數下限 SHALL 隨本次新增的委託解封文案上修，SHALL NOT 停留在舊值。
...
- **THEN** 無任一值含狀態機器碼、內部函式名、行話黑名單詞或已退場用詞，且葉鍵數未低於釘定下限
...
#### Scenario: 委託模式顯示對應的不可救援陳述
- **WHEN** 委託模式的部署未解封，解封頁渲染完成
- **THEN** MUST 呈現「主金鑰由外部保管處持有、失去即全部資料不可解、系統不提供救援」的陳述，其版面優先度 MUST 與本地模式的遺失警語相同

#### Scenario: 操作者所需事實未因改寫而減少
- **WHEN** 檢視既有部署解封、全新安裝初始化與守衛攔下三條路徑的文案（含展開區展開後）
- **THEN** 「解封前要先用平常登入的帳號密碼確認身分」「服務暫停期間不核對動態驗證碼」「憑證只留在後端記憶體，封存或重啟後要重新提供」
  「送出憑證前先核對保管處位址與金鑰」「連續失敗會暫停嘗試、倒數結束自動恢復」「主金鑰的三種輸入寫法」
  「另一實例仍在執行時停止它即可」「確認的後果由確認者承擔」MUST 各自仍可自畫面讀到

#### Scenario: 已退場用詞不再出現
- **WHEN** 掃描三份 locale 的服務前頁面與金鑰管理相關鍵
- **THEN** MUST NOT 出現「封印」「還原」作為狀態或動作的稱呼，檢查點鏈概念 MUST 稱為存證

#### Scenario: 三語警告強度一致
- **WHEN** 以三語檢視遺失警語、保存確認勾選項、保管處核對勾選項與守衛攔下頁的承擔勾選項
