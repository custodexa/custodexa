...
注入終端與錄影的阻斷警告，在由自動化主體以整行送入（無逐字回顯）的會話中 SHALL 帶被攔下的指令原文；人類逐字鍵入的會話因回顯已留痕，警告內容維持規則名稱即可。
...
#### Scenario: 自動化主體的阻斷警告帶指令原文
- **WHEN** agent 會話整行送入命中 block 規則的指令
- **THEN** 錄影中的阻斷警告含規則名稱與該指令原文；同一規則由人類逐字鍵入時警告內容不變
