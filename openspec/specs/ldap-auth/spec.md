...
### Requirement: 目錄群組屬性的設定與取得


目錄設定 SHALL 可指定承載群組成員資格的屬性名稱。未指定 SHALL 等同該目錄的群組映射關閉：登入路徑 SHALL NOT 因此向目錄索取額外屬性。該目錄同時無任何啟用中的映射規則時，登入路徑 SHALL NOT 開啟任何額外交易、SHALL NOT 寫入任何審計列——此為「未啟用映射的部署零影響」的保證。該目錄已有啟用中的映射規則時，登入 SHALL 依映射能力的規定寫入跳過事件（規則設好但屬性名未設是靜默零命中的唯一成因，須有訊號）。

已指定時，登入的目錄搜尋 SHALL 於既有的屬性請求清單中一併索取該屬性，SHALL NOT 為此另發一次搜尋。認證結果 SHALL 攜帶取得的群組值集合，並 SHALL 攜帶一個「本次是否成功取得群組資料」的旗標供上層判定。

**已知的判準 SHALL 為「屬性名已設定且搜尋成功」**。命中的目錄項目不含該屬性 SHALL 視為空集合，SHALL NOT 視為未知：目錄協定不回傳零值屬性，「不屬任何群組」與「搜尋帳號無權讀取該屬性」在單筆項目上同形，於此層無法區分。若採「未知即保留」，被移出最後一個群組的使用者將永不失去映射角色——那正是本能力要解的情形。

群組屬性與使用者屬性由同一次搜尋取得，搜尋失敗即認證失敗：登入 SHALL 被拒並以登入失敗留痕，不進入映射重算。「未知」SHALL 只指已認證但群組屬性未成功取得的情形（「已成功取得」旗標為假）。屬性值 SHALL 原樣保留、SHALL NOT 做未經說明的正規化——比對規則由映射能力定義，此層只負責取得。

登入路徑的群組取得 SHALL 沿用既有的單次設定快照與傳輸政策閘：SHALL NOT 另建撥號路徑、SHALL NOT 繞過出站位址政策。

#### Scenario: 未設定屬性名即完全短路

- **WHEN** 目錄設定未填群組屬性名、該目錄無任何啟用中的映射規則，而使用者經目錄登入
- **THEN** 目錄搜尋的屬性請求清單與設定前相同，登入未觸發任何角色列讀寫與任何映射相關審計

#### Scenario: 一次搜尋取回群組

- **WHEN** 目錄設定已填群組屬性名而使用者經目錄登入
- **THEN** 群組值隨既有的使用者搜尋一併取回，登入路徑對目錄的請求次數不因此增加

#### Scenario: 項目無該屬性即空集合

- **WHEN** 屬性名已設定、搜尋成功，而命中的目錄項目不含該屬性
- **THEN** 結果為空集合且「已成功取得」旗標為真

#### Scenario: 搜尋失敗即認證失敗

- **WHEN** 屬性名已設定但目錄搜尋失敗
- **THEN** 登入被拒並寫入登入失敗記錄，映射重算未執行，既有映射列與憑證世代不變

#### Scenario: 已認證但群組屬性未成功取得即未知

- **WHEN** 認證成功而群組屬性的「已成功取得」旗標為假
- **THEN** 上層據此保留既有映射列、不推進世代並寫入可辨識的未知事件

### Requirement: 連線測試驗證群組屬性可讀


目錄連線測試的屬性抽樣 SHALL 於既有的兩項之外，一併回報抽樣項目的群組屬性是否有值，使管理者能在啟用映射前確認屬性名正確與搜尋帳號具備讀取權限。

該回報 SHALL 維持只回布林、SHALL NOT 回傳屬性值：抽樣的用途是回答「映射設對了嗎」，回傳值會使連線測試成為不需目標帳號認證即可讀取目錄內容的管道。

目錄設定未填群組屬性名時，該欄 SHALL 明確表達「未設定」而非「無值」——兩者的處置不同（前者為映射關閉，後者需管理者檢查權限）。

#### Scenario: 抽樣回報群組屬性有值

- **WHEN** 管理者對已填群組屬性名的目錄設定執行連線測試，且抽樣項目具該屬性
- **THEN** 測試結果回報該屬性有值，回應中不含任何屬性值

#### Scenario: 屬性名打錯可辨識

- **WHEN** 群組屬性名填成目錄上不存在的名稱並執行連線測試
- **THEN** 測試結果回報該屬性無值，管理者據此知道啟用映射會使全體使用者被判為不屬任何群組

#### Scenario: 未設定與無值可區分

- **WHEN** 目錄設定未填群組屬性名並執行連線測試
- **THEN** 結果的群組屬性欄表達為「未設定」，與「已設定但抽樣無值」可區分
