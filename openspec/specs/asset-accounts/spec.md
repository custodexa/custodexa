...
以批次改密的「整批同一組」模式成功提交新憑證時，系統 SHALL 使該帳號於脫離原群組後歸入該批次的群組（識別隨批次產生）；
候選經系統重試轉正者 SHALL 同樣歸入。批次結束時與候選轉正時，該群組成員少於 2 且該批次已無待驗證候選者 SHALL 解散。
...
- **WHEN** 甲原與丙同群組，甲與乙以整批同一組模式改密成功
- **THEN** 甲與乙同屬批次群組；丙因原群組只剩一員而脫離
