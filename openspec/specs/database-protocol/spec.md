...
**環境變數 SHALL NOT 用於傳遞憑證（含「為了省事暫時改回」）**：程序環境區塊以 NUL 分隔、文字讀取只取得到第一段，這個性質只涵蓋文字讀取路徑，**不成立於二進位讀取原語**。CLI 具備的二進位讀取原語可完整讀出子程序的環境區塊，其中的憑證即可被取得；且因各會話共用同一降權身分，**跨會話亦成立**，只被授權某一資產的使用者將可取得另一資產的密碼。以環境變數傳遞憑證即違反連線收口紅線（前端零接觸明文憑證）。
...
**記憶體殘留是偶然，SHALL NOT 被當成防線**：密碼在注入後仍存在於 client 的程序記憶體。CLI 既有的讀取原語目前讀不出該記憶體，是原語讀取方式的偶然限制，**不是** client 或核心提供的保護，SHALL NOT 據以宣稱記憶體面已受保護。本要求可驗證的保證範圍到「憑證不在環境、不在 argv、不在任何可讀的一般檔」為止；要消除記憶體殘留需憑證代理（後端代持連線），屬未來範圍。

本要求另 SHALL 以「該降權身分讀得到的一般檔裡沒有憑證」為前提：各 client 的讀檔原語可完整讀出該身分有讀取權的任何一般檔，該前提由「CLI 執行環境的可驗證不變式」的憑證掃描承擔。
...
1. **程序資訊面**：各會話彼此讀得到對方 CLI 子程序的程序資訊（環境區塊與命令列），且以二進位讀取原語可取得**完整內容**，SHALL NOT 再以「NUL 截斷只洩漏第一段」描述（該說法只對文字讀取成立）。判準：其中 SHALL NOT 含憑證（見本要求主文），可見者限於對方會話的程式名、連線參數（主機／埠／使用者名）與最小化後的環境變數名。對方的檔案描述符清單亦可列舉，此處 SHALL NOT 宣稱其依賴 ptrace 能力的缺席；擋下跨會話 fd 讀寫的是 PTY 從屬端裝置的檔案權限，降權身分對它 SHALL 無任何權限。該權限 SHALL 有守衛測試（權限一旦放寬給該身分即轉紅），否則跨會話面會從「讀到環境變數名」升級為「讀他人擊鍵與注入他人終端」。
2. **共用暫存區的檔案交換面**：CLI 執行環境有一塊由容器執行期提供、映像內無法變更的記憶體檔案系統，是該身分僅有的可寫面；一個會話可經 CLI 的匯出原語在此落檔，另一個會話可經匯入原語原樣取回，各 client 皆有同等原語。**能傳什麼**：任一會話的使用者權限內查得到的任意資料，上限為該檔案系統容量。**稽核看得到什麼**：兩側的命令文字皆進 `session_commands`（誰在何時對哪個檔案做了讀寫）。**看不到什麼**：檔案內容不經終端，故不入錄影與虛擬螢幕；容器重啟即消失，事後無從取證。此為刻意接受的殘留（封閉它需 per-session 暫存區或檔案層攔截），SHALL NOT 被描述為「已封閉」。
...
新增任何以本地 CLI 子程序代理的協議時，SHALL 假設該 client 具備**任意本地讀取能力**（檔案與程序記憶體），SHALL NOT 以「本 client 沒有讀檔命令」或「原生 sandbox 會擋」作為憑證面的論證。理由是機械性的：這類能力散落在 client 的各種功能中（psql 的 `\lo_import`／`\copy`、mariadb 的 `LOAD DATA LOCAL INFILE`／`source`、sqlcmd 的 `:R`／`:OUT`），逐一枚舉並追上上游版本演進在原理上做不到；「輸入層過濾擋得住」與「NUL 截斷只洩漏第一段」同屬這類不成立的論證。
